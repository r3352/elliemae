...
// Type: Elli.Data.MongoDB.Mapping.LogRecordMap
...
{
  public class LogRecordMap
  {
    public static void Register()
    {
      if (BsonClassMap.IsClassMapRegistered(typeof (LogRecord)))
        return;
      BsonClassMap.RegisterClassMap<LogRecord>((Action<BsonClassMap<LogRecord>>) (cm =>
      {
        cm.AutoMap();
        cm.MapField("alerts").SetElementName("Alerts");
        cm.MapField("commentList").SetElementName("CommentList");
        cm.UnmapProperty<Loan>((Expression<Func<LogRecord, Loan>>) (c => c.Loan));
      }));
      LogAlertMap.Register();
      LogCommentMap.Register();
      GoodFaithFeeVarianceCureLogMap.Register();
    }
  }
}
