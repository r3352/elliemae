// Decompiled with JetBrains decompiler
// Type: ClosingMarket.WebServices.EnterpriseServiceData
// Assembly: ClosingMarket.WebServices, Version=1.0.2749.29102, Culture=neutral, PublicKeyToken=null
// MVID: 510652A0-EF36-486C-9EB6-CECE9FC11560
// Assembly location: C:\SmartClientCache\Apps\UAC\Ellie Mae\xIHR5EqGa7zPnRG0YpD5z4TPAB0=\Encompass360\ClosingMarket.WebServices.dll

using System.Xml.Serialization;

#nullable disable
namespace ClosingMarket.WebServices
{
  [SoapType("EnterpriseServiceData", "http://www.closingmarket.com")]
  public class EnterpriseServiceData : SerializableObject
  {
    public int EnterpriseServiceID;
    public string ServiceName;
    public string Description;
    public EnterpriseServiceCategory Category;
    public int EnterpriseID;
    public string InformationUrl;
    public EnterpriseServiceInterfaceType InterfaceType;
    public string UserName;
    [SoapElement(DataType = "base64Binary")]
    public byte[] Password;
    public string Destination;
    public string QueryServiceUrl;
    public string CCEUtilID;
    public string ServerName;
    public string DatabaseName;
    public string RoutingData;
  }
}
