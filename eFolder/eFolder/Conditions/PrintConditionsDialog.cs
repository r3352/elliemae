...
// Type: EllieMae.EMLite.eFolder.Conditions.PrintConditionsDialog
...
{
  public class PrintConditionsDialog : Form
  {
    ...
    private void printFiles()
    {
      ...
      foreach (ConditionLog cond in this.condList)
      {
        foreach (DocumentLog documentLog in allDocuments)
        {
          if (documentLog.Conditions.Contains(cond) && !documentLogList.Contains(documentLog))
            documentLogList.Add(documentLog);
        }
      }
      using (SelectDocumentsDialog selectDocumentsDialog = new SelectDocumentsDialog(this.loanDataMgr, allDocuments, documentLogList.ToArray(), this.defaultStackingEntry, SelectDocumentsReasonType.PrintSave))
      {
    ...
    {
      string filepath = new FormExport(this.loanDataMgr).ExportDetails(this.condList);
      if (filepath == null)
        return;
    ...
    private void InitializeComponent()
    {
      ...
      this.rdoDetails.Size = new Size(126, 18);
      this.rdoDetails.TabIndex = 5;
      this.rdoDetails.Text = "Print condition details";
      ...
      this.Name = nameof (PrintConditionsDialog);
      this.ShowInTaskbar = false;
      this.StartPosition = FormStartPosition.CenterParent;
      this.Text = "Print Conditions";
