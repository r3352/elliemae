...
// Type: EllieMae.EMLite.Server.Query.SecurityTradeFieldTranslator
...
{
  internal class SecurityTradeFieldTranslator : ICriterionTranslator
  {
    ...
    private TradeFieldTranslator loanTradeTranslator;

    public SecurityTradeFieldTranslator() => this.loanTradeTranslator = new TradeFieldTranslator();
    ...
    public CriterionName TranslateName(string fieldName)
    {
      ...
      if (criterionName.FieldSource == "")
        criterionName = new CriterionName("SecurityTradeDetails", criterionName.FieldName);
      return this.loanTradeTranslator.TranslateName(criterionName.ToString());
    }

    public QueryCriterion TranslateCriterion(QueryCriterion cri)
    {
      return this.loanTradeTranslator.TranslateCriterion(cri);
    }
  }
}
