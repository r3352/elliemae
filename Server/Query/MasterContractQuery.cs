...
// Type: EllieMae.EMLite.Server.Query.MasterContractQuery
...
{
  internal class MasterContractQuery : QueryEngine
  {
    ...
    public MasterContractQuery(UserInfo currentUser)
      : base(ClientContext.GetCurrent().Settings.ConnectionString, ClientContext.GetCurrent().Settings.DbServerType, currentUser, (ICriterionTranslator) new MasterContractFieldTranslator())
    {
    ...
    public override string PrimaryKeyTableIdentifier => "MasterContracts";

    public override string PrimaryKeyIdentifier => "MasterContracts.ContractID";

    public override string UserAccessQueryKeyColumnName => "ContractID";
    ...
    public override FieldSource GetFieldSource(string name)
    {
      ...
      {
        case "contracttrades":
          return new FieldSource("ContractTrades", "left outer join FN_GetContractLoanSummaryInline() ContractTrades on MasterContracts.ContractID = ContractTrades.ContractID", new string[1]
          {
            "MasterContract"
          });
        case "loantradedetails":
          return new FieldSource("LoanTradeDetails", "left outer join LoanTradeDetails LoanTradeDetails on LoanTradeDetails.ContractID = ContractTrades.ContractID", new string[1]
          {
            "ContractTrades"
          });
        case "mbspooldetails":
          return new FieldSource("MbsPoolDetails", "left outer join MbsPoolDetails MbsPoolDetails on MbsPoolDetails.ContractID = ContractTrades.ContractID", new string[1]
          {
            "ContractTrades"
          });
        default:
    ...
    public override void SplitFiltersByReportsFor(FieldFilterList filterList)
    {
      foreach (FieldFilter filter in (List<FieldFilter>) filterList)
      {
        ...
        {
          if (!this.Filters.Contains((object) ReportsFor.MasterContracts))
            this.Filters.Add((object) ReportsFor.MasterContracts, (object) new FieldFilterList());
          (this.Filters[(object) ReportsFor.MasterContracts] as FieldFilterList).Add(filter);
        }
    ...
    {
      return this.Filters.Contains((object) ReportsFor.MasterContracts) ? this.Filters[(object) ReportsFor.MasterContracts] as FieldFilterList : new FieldFilterList();
    }
      ...
      bool isExternalOrganization)
    {
      ...
      if (this.Filters.Contains((object) ReportsFor.LoanTrades))
      {
        ...
        if (this.Filters.Contains((object) ReportsFor.MasterContracts))
          newFilterList1.AddRange((IEnumerable<FieldFilter>) (this.Filters[(object) ReportsFor.MasterContracts] as FieldFilterList));
      ...
      if (this.Filters.Contains((object) ReportsFor.MBSPools))
      {
        ...
        if (this.Filters.Contains((object) ReportsFor.MasterContracts))
          newFilterList2.AddRange((IEnumerable<FieldFilter>) (this.Filters[(object) ReportsFor.MasterContracts] as FieldFilterList));
    ...
    public override void GetCategories(List<ColumnInfo> fields)
    {
      foreach (ColumnInfo field in fields)
      {
        ...
        {
          if (!this.Categories.Contains(ReportsFor.MasterContracts))
            this.Categories.Add(ReportsFor.MasterContracts);
        }
    ...
    {
      return reportsFor == ReportsFor.MasterContracts;
    }
    ...
    public override string GetChildrenTableJoins(ReportsFor reportsFor, string tableJoins)
    {
      ...
      if (reportsFor == ReportsFor.LoanTrades)
      {
        ...
        {
          if (str.Contains(" LoanTradeDetails LoanTradeDetails ") && this.Filters.Contains((object) ReportsFor.LoanTrades))
            stringBuilder.AppendLine(str + " and LoanTradeDetails.TradeID in (select TradeID from #loantrade_ids)");
          else if (!str.Contains("MbsPoolDetails") && !str.Contains("FN_GetMbsPoolSummaryInline(1)"))
      ...
      if (reportsFor == ReportsFor.MBSPools)
      {
        foreach (string str in strArray)
        {
          if (str.Contains(" MbsPoolDetails MbsPoolDetails ") && this.Filters.Contains((object) ReportsFor.MBSPools))
            stringBuilder.AppendLine(str + " and MbsPoolDetails.TradeID in (select TradeID from #mbspool_ids)");
          else if (!str.Contains(" SecurityTradeDetails SecurityTradeDetails ") && !str.Contains("LoanTradeDetails") && !str.Contains("FN_GetLoanTradeSummaryInline(1)"))
            stringBuilder.AppendLine(str);
        }
      }
