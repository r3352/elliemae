...
// Type: EllieMae.EMLite.Server.ServiceObjects.KafkaEvent.KafkaProcessor
...
{
  public class KafkaProcessor : IMessageQueueProcessor, IContextBoundObject
  {
    ...
    {
      this.Publish(messageRoutingKey, payload, messageQueueEventParameters, new KafkaEventResponseHandler());
    }
    ...
    private void Publish(
      ...
      string payload,
      MessageQueueEventParameters messageQueueEventParameters,
      KafkaEventResponseHandler reportHandler)
    {
      string str = "Before sending kafka message {0} message for event: {1}, instanceId: {2}, loanId: {3}, correlationId: {4}, {0}: {5}, publishedTime: {6}, processOperation: {7}";
      ...
      if (KafkaUtils.IsKafkaWarningLogEnabled && messageQueueEventParameters.MessageType.Equals("TradeSync"))
      {
        ...
        TraceLog.WriteWarning(nameof (KafkaProcessor), string.Format(str, (object) messageQueueEventParameters.MessageType, (object) queueMessage.Type, (object) messageQueueEventParameters.InstanceId, (object) queueMessage.LoanId, (object) queueMessage.CorrelationId, (object) queueMessage.LoanPath, (object) queueMessage.PublishTime, KafkaUtils.IsAsyncEnabled ? (object) "asynchronous" : (object) "synchronous"));
      }
      IProducer<string, string> staticProducer = KafkaProducer.StaticProducer;
      string topic = messageRoutingKey;
      Message<string, string> message = new Message<string, string>();
      message.Key = messageQueueEventParameters.QueueMessage.LoanId;
      ...
      {
        TraceLog.WriteInfo(nameof (KafkaProcessor), string.Format("Entering continue with task status {0} for the message {1}", (object) taskResult.Status.ToString(), (object) objectStateParam.ToString()));
        if (!taskResult.IsCompleted)
          return;
        if (taskResult.Exception == null && taskResult.Result != null && taskResult.Result.Message != null)
        {
          reportHandler.WriteSucessLog(taskResult.Result.Message.Value, objectStateParam.ToString(), taskResult.Result.Topic);
        }
        else
        {
          if (taskResult.Exception == null)
            return;
          DeliveryResult<string, string> deliveryResult = ((ProduceException<string, string>) taskResult.Exception.InnerExceptions[0]).DeliveryResult;
          reportHandler.FailedMessageHandler(deliveryResult.Value, objectStateParam.ToString(), deliveryResult.Topic, taskResult.Exception);
        }
      }), (object) JsonConvert.SerializeObject((object) messageQueueEventParameters));
      string logFormat = "Sent kafka message {0} message for event: {1}, instanceId: {2}, loanId: {3}, correlationId: {4}, {0}: {5}, publishedTime: {6}";
    ...
    private void PublishAsync(MessageQueueEvent messageQueueEvent)
    {
      foreach (QueueMessage queueMessage in messageQueueEvent.QueueMessages)
      {
        ...
        KafkaEventResponseHandler reportHandler = new KafkaEventResponseHandler();
        try
        {
          str = messageQueueEvent.CreatePayload(queueMessage);
          this.Publish(topic, str, messageQueueEventParameters, reportHandler);
        }
        catch (ProduceException<string, string> ex)
        {
          reportHandler.FailedMessageHandler(str, JsonConvert.SerializeObject((object) messageQueueEventParameters), messageQueueEventParameters.Topic, new AggregateException(new Exception[1]
          {
            (Exception) ex
          }));
        }
      ...
      string logFormat)
    {
      ...
      TraceLog.WriteInfo(nameof (KafkaProcessor), message);
    }
    ...
    public void handler(DeliveryReport<string, string> deliveryReport)
    {
      ...
      using (ctx.MakeCurrent((IDataCache) null, (string) null, new Guid?(), new bool?()))
      {
        ...
        if (!code.Equals((object) ErrorCode.NoError))
        {
          ...
          TraceLog.WriteError(nameof (KafkaProcessor), string.Format("Publishing kafka message failed for the event: Reason: {0}, ErrorCode: {1}, IsBrokerError: {2}, IsLocalError : {3}", objArray));
        ...
        else
        {
          ...
          TraceLog.WriteInfo(nameof (KafkaProcessor), string.Format("Publishing {0} message for event: {1}, instanceId: {2}, loanId: {3}, correlationId: {4}, {0}: {5}, publishedTime: {6}", (object) this.queueEventParameters.MessageType, (object) queueMessage.Type, (object) this.queueEventParameters.InstanceId, (object) queueMessage.LoanId, (object) queueMessage.CorrelationId, (object) queueMessage.LoanPath, (object) queueMessage.PublishTime));
        }
      }
    }
