...
namespace EllieMae.Encompass.AsmResolver.Utils
{
  internal class XT
  {
    ...
    private static void c(uint[] v, uint[] k)
    {
      ...
      while (num4-- > 0U)
      {
        num1 += (uint) (((int) num2 << 4 ^ (int) (num2 >> 5)) + (int) num2 ^ (int) num3 + (int) k[(IntPtr) (num3 & 3U)]);
        num3 += 2654435769U;
        num2 += (uint) (((int) num1 << 4 ^ (int) (num1 >> 5)) + (int) num1 ^ (int) num3 + (int) k[(IntPtr) (num3 >> 11 & 3U)]);
      }
      v[0] = num1;
      v[1] = num2;
    }
