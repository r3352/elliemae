...
using EllieMae.EncompassAPI.WebServices;
using System;
using System.Net;
  ...
  public class SmartClientUtils
  {
    ...
    public static string GetAttribute(string clientID, string appName, string attrName)
    {
      for (int index = 0; index < 2; ++index)
      {
        try
        {
          return new SmartClientService(SmartClientUtils.DefaultSmartClientServiceUrl).GetAttribute(clientID, "", appName, attrName);
        }
        catch (Exception ex)
        {
