...
// Type: Encompass.Export.Freddie30Validate
...
{
  internal class Freddie30Validate
  {
    private IBam loanData;
    private RequiredList missingFields = new RequiredList();

    internal Freddie30Validate(IBam loanData) => this.loanData = loanData;
    ...
    private void CheckLP()
    {
      ...
      this.checkBlankField("FR0112", "Years at present address");
      ...
      if (this.loanData.GetUserSetting("LP", "TPONo") == string.Empty)
      {
        this.checkBlankUserSetting("SellerNo", "Freddie Mac Seller No. - See 'LOS Settings' in ePASS");
        this.checkLengthOfUserSetting("SellerNo", "Freddie Mac Seller No. must be 6 digits - See 'LOS Settings' in ePASS", 6);
      }
      this.checkBlankUserSetting("Password", "Agency / TPO Password - See 'LOS Settings' in ePASS");
      ...
      if (this.loanData.GetSimpleField("CASASRN.X89") == "1" || this.loanData.GetSimpleField("CASASRN.X89") == "2")
      {
        this.checkBlankField("CASASRN.X3", "Order mortgage insurance (company)");
        this.checkBlankField("324", "Company phone");
        this.checkBlankField("326", "Company fax");
        this.checkBlankField("430", "MI coverage percent");
        if (this.loanData.GetSimpleField("315") != string.Empty)
          this.checkBlankField("1256", "Lender contact name");
      }
      if (flag)
      {
      ...
      if (!(this.loanData.GetSimpleField("CASASRN.X88") == "Y"))
        return;
      this.checkBlankField("CASASRN.X2", "Merged credit company");
      if (this.loanData.GetNumberOfBorrowerPairs() <= 1)
        return;
      this.missingFields.Add("CASASRN.X88", "Merged Credit cannot be ordered when there are ");
      this.missingFields.Add("", "co-mortgagors. Merged Credit will NOT be ordered if you");
      this.missingFields.Add("", "continue.");
    }
