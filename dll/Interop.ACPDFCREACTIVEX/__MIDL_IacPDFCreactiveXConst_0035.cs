// Decompiled with JetBrains decompiler
// Type: ACPDFCREACTIVEX.__MIDL_IacPDFCreactiveXConst_0035
// Assembly: Interop.ACPDFCREACTIVEX, Version=3.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 4DA5DA38-3850-4E97-8521-1A0336DA34F6
// Assembly location: C:\SmartClientCache\Apps\Ellie Mae\Encompass\Interop.ACPDFCREACTIVEX.dll

#nullable disable
namespace ACPDFCREACTIVEX
{
  public enum __MIDL_IacPDFCreactiveXConst_0035
  {
    acCommandToolStatusDisabled = 0,
    acCommandToolStatusUnknown = 0,
    acCommandToolStatusEnabled = 1,
    acCommandToolStatusPressed = 2,
    acCommandToolStatusHidden = 4,
  }
}
