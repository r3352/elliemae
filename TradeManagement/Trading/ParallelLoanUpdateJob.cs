...
// Type: EllieMae.EMLite.Trading.ParallelLoanUpdateJob
...
{
  public class ParallelLoanUpdateJob : ITradeJob
  {
    ...
    private AssignmentItem assignmentItm;
    ...
    {
      Tracing.Log(ParallelLoanUpdateJob.sw, nameof (ParallelLoanUpdateJob), TraceLevel.Verbose, "In StartAsync. AssignmentItems.Count: " + (object) this.AssignmentItems.Count);
      ...
      {
        Tracing.Log(ParallelLoanUpdateJob.sw, nameof (ParallelLoanUpdateJob), TraceLevel.Verbose, "Calling Task.Factory.StartNew for index " + (object) state);
    ...
    private void processLoan(object assignmentIndexParam)
    {
      try
      {
        Tracing.Log(ParallelLoanUpdateJob.sw, nameof (ParallelLoanUpdateJob), TraceLevel.Verbose, "In processLoan for index " + (object) (int) assignmentIndexParam);
        if (this.cancellationRequested)
        {
          Tracing.Log(ParallelLoanUpdateJob.sw, nameof (ParallelLoanUpdateJob), TraceLevel.Verbose, "In processLoan, cancellation was requested for index " + (object) (int) assignmentIndexParam);
      ...
      {
        Tracing.Log(ParallelLoanUpdateJob.sw, nameof (ParallelLoanUpdateJob), TraceLevel.Error, "Error in processLoan for index " + (object) (int) assignmentIndexParam + ": " + ex.ToString());
      }
    }
    ...
    public void StartAsyncBackground()
    {
      ...
      this.onStarted(new TradeUpdateEventArgs("", (object) null));
      Tracing.Log(ParallelLoanUpdateJob.sw, nameof (ParallelLoanUpdateJob), TraceLevel.Verbose, "Using Parallel ForEach");
      List<AssignmentItem> assignmentItems = this.AssignmentItems;
      ParallelOptions parallelOptions = new ParallelOptions();
      parallelOptions.MaxDegreeOfParallelism = 10;
      Action<AssignmentItem> body = (Action<AssignmentItem>) (item =>
      {
        Tracing.Log(ParallelLoanUpdateJob.sw, nameof (ParallelLoanUpdateJob), TraceLevel.Verbose, "Processing loan guid = " + item.Assignment.Guid);
        this.processLoanBackground(item);
      });
      Parallel.ForEach<AssignmentItem>((IEnumerable<AssignmentItem>) assignmentItems, parallelOptions, body);
      if (this.cancellationRequested)
    ...
    private void processLoanBackground(AssignmentItem assignmentParam)
    {
      if (this.cancellationRequested)
      {
        this.incrementLoansSkipped();
    ...
    private void updateLoan(AssignmentItem assignmentItm)
    {
      try
      {
        if (Session.LoanData != null && assignmentItm.Assignment.Guid == Session.LoanData.GUID)
        {
          if (assignmentItm.Assignment.PendingStatus == LoanTradeStatus.Unassigned)
            Session.LoanTradeManager.CommitPendingTradeStatus(assignmentItm.LoanTrade.TradeID, assignmentItm.Assignment.Guid, assignmentItm.Assignment.PendingStatus, assignmentItm.Assignment.Rejected);
        ...
        if (!new TradeProcesses2().WorkOneLoan(assignmentItm.Assignment, assignmentItm.LoanTrade, this.ForceUpdateOfAllLoans, this.SkipFieldList, assignmentItm.SecurityPrice))
          this.incrementLoansSkipped();
        this.incrementLoansCompleted();
      }
      ...
      {
        Tracing.Log(ParallelLoanUpdateJob.sw, nameof (ParallelLoanUpdateJob), TraceLevel.Error, "Error in updateLoan for " + assignmentItm.Assignment.Guid + ": " + ex.ToString());
        this.hadErrors = true;
