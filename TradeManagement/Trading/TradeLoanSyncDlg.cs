...
// Type: EllieMae.EMLite.Trading.TradeLoanSyncDlg
...
{
  public class TradeLoanSyncDlg : Form
  {
    ...
    public TradeLoanSyncDlg(TradePriceAdjustments tradeAdjList, LoanTradeInfo trade)
    {
      ...
      this.chBCheckAll.CheckedChanged += new EventHandler(this.chBCheckAll_CheckedChanged);
      this.chbApplyAll.Checked = Utils.ParseBoolean(Session.StartupInfo.TradeSettings[(object) "Trade.LoanTradeUpdateAllLoans"]);
    }
    ...
    private void initMapNEW()
    {
      ...
      {
        if (!(row["TradeType"].ToString() != "2") && (!(row["FieldId"].ToString() == "3888") || this.trade.ServicingType == ServicingType.ServicingRetained))
    ...
    private void InitializeComponent()
    {
      ...
      this.Name = nameof (TradeLoanSyncDlg);
