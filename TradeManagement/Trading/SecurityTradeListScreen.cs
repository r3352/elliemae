...
// Type: EllieMae.EMLite.Trading.SecurityTradeListScreen
...
{
  public class SecurityTradeListScreen : UserControl, IMenuProvider
  {
    private const string className = "SecurityTradeListScreen";
    private static readonly string sw = Tracing.SwOutsideLoan;
    private SecurityLoanTradeStatusEnumNameProvider tradeStatusNameProvider = new SecurityLoanTradeStatusEnumNameProvider();
    ...
    private SecurityTradeReportFieldDefs tradeFieldDefs;
    private SecurityTradeView currentView;
    ...
    public SecurityTradeListScreen()
    {
    ...
    {
      this.resetFieldDefs();
      this.loadViewList(Session.ConfigurationManager.GetTemplateDirEntries(EllieMae.EMLite.ClientServer.TemplateSettingsType.SecurityTradeView, FileSystemEntry.PrivateRoot(Session.UserID)));
      string privateProfileString = Session.GetPrivateProfileString("SecurityTrades", "DefaultView");
    ...
    private void loadViewList(FileSystemEntry[] fsEntries)
    {
      ...
      {
        Tracing.Log(SecurityTradeListScreen.sw, nameof (SecurityTradeListScreen), TraceLevel.Error, "Error loading view list: " + (object) ex);
    ...
    {
      this.tradeFieldDefs = SecurityTradeReportFieldDefs.GetFieldDefs();
    }
    ...
    public void RefreshContents()
    {
      if (this.gvLayoutManager == null)
        this.init();
      if (string.IsNullOrEmpty(Session.ConfigurationManager.GetCompanySetting("MIGRATION", "TradeSecurityTrade14.2.0.3") ?? ""))
      {
        int num = (int) new MigrateSecurityTradeDataDialog().ShowDialog();
      }
    ...
    public void RefreshList(bool preserveSelections)
    {
      ...
      {
        using (PerformanceMeter performanceMeter = PerformanceMeter.StartNew("SecurityTrades.Refresh", "Refresh the Security Trade screen data", true, 148, nameof (RefreshList), "D:\\ws\\24.3.0.0\\EmLite\\TradeManagement\\Trading\\SecurityTradeListScreen.cs"))
        {
    ...
    private void displayCurrentPage(bool preserveSelections)
    {
      ...
      SecurityTradeViewModel[] securityTradeViewModelArray = new SecurityTradeViewModel[0];
      if (currentPageItemCount > 0)
        securityTradeViewModelArray = (SecurityTradeViewModel[]) this.tradeCursor.GetItems(currentPageItemIndex, currentPageItemCount);
      ...
      for (int index = 0; index < securityTradeViewModelArray.Length; ++index)
      {
        GVItem gvItem = this.createGVItem(securityTradeViewModelArray[index]);
        this.gvTradeList.Items.Add(gvItem);
        if (dictionary.ContainsKey(securityTradeViewModelArray[index].TradeID))
    ...
    private GVItem createGVItem(SecurityTradeViewModel tradeInfo)
    {
    ...
    private void retrieveTradeData(QueryCriterion filter, SortField[] sortFields)
    {
      ...
      try
      {
        ...
        QueryCriterion queryCriterion = fieldFilterList.CreateEvaluator().ToQueryCriterion();
        if (filter == null)
          filter = queryCriterion;
        else
          filter = QueryCriterion.Join(new QueryCriterion[2]
          {
            queryCriterion,
            filter
          }, BinaryOperator.And);
        ...
        if (filter == null)
          this.tradeCursor = Session.SecurityTradeManager.OpenTradeCursor(new QueryCriterion[0], sortFields, fieldList, true, false);
        else
          this.tradeCursor = Session.SecurityTradeManager.OpenTradeCursor(new QueryCriterion[1]
          {
            filter
    ...
    {
      TradeManagementConsole.Instance.OpenNewSecurityTrade();
    }
    ...
    {
      SecurityTradeViewModel tag = (SecurityTradeViewModel) item.Tag;
      TradeManagementConsole.Instance.OpenSecurityTrade(tag.TradeID);
    }
    ...
    private void btnDuplicate_Click(object sender, EventArgs e)
    {
      ...
      {
        SecurityTradeViewModel tag = (SecurityTradeViewModel) this.gvTradeList.SelectedItems[0].Tag;
        SecurityTradeInfo trade1 = Session.SecurityTradeManager.GetTrade(tag.TradeID);
        if (tag == null)
        {
        ...
        {
          SecurityTradeInfo trade2 = trade1.Duplicate();
          TradeManagementConsole.Instance.OpenSecurityTrade(trade2);
        }
      }
    }
    ...
    private void btnLock_Click(object sender, EventArgs e)
    {
      ...
      {
        bool locked = ((SecurityTradeViewModel) this.gvTradeList.SelectedItems[0].Tag).Locked;
        for (int index = 1; index < this.gvTradeList.SelectedItems.Count; ++index)
        {
          if (((SecurityTradeViewModel) this.gvTradeList.SelectedItems[0].Tag).Locked != locked)
          {
        ...
        try
        {
          ...
          {
            this.toggleLockState((SecurityTradeViewModel) selectedItem.Tag);
            this.RefreshList(true);
          }
        }
        catch (Exception ex)
    ...
    private void toggleLockState(SecurityTradeViewModel info)
    {
      SecurityTradeInfo trade = Session.SecurityTradeManager.GetTrade(info.TradeID);
      if (trade == null || trade.Locked != info.Locked)
        return;
      trade.Locked = !trade.Locked;
      Session.SecurityTradeManager.UpdateTrade(trade);
    ...
    private void btnDelete_Click(object sender, EventArgs e)
    {
      ...
      else
      {
        try
        {
          ...
          {
            if (Session.SessionObjects.SecurityTradeManager.GetTradeAssigments(((TradeBase) this.gvTradeList.SelectedItems[index].Tag).TradeID).Length != 0)
            {
              flag = false;
              break;
            }
          ...
          if (!flag)
          {
            int num2 = (int) Utils.Dialog((IWin32Window) this, "To delete the security trade(s), loan trades assigned to the security trade(s) must be unassigned first.", MessageBoxButtons.OK, MessageBoxIcon.Hand);
            return;
          }
        }
        catch (Exception ex)
        ...
        try
        {
          SecurityTradeViewModel[] state = new SecurityTradeViewModel[this.gvTradeList.SelectedItems.Count];
          for (int index = 0; index < this.gvTradeList.SelectedItems.Count; ++index)
            state[index] = (SecurityTradeViewModel) this.gvTradeList.SelectedItems[index].Tag;
        ...
        {
          int num5 = (int) Utils.Dialog((IWin32Window) this, "An unexpected error has occurred while attempting to delete the trade(s): " + (object) ex, MessageBoxButtons.OK, MessageBoxIcon.Hand);
        }
    ...
    private DialogResult deleteTradesAsync(object tradesAsObj, IProgressFeedback feedback)
    {
      try
      {
        SecurityTradeViewModel[] securityTradeViewModelArray = (SecurityTradeViewModel[]) tradesAsObj;
        int num1 = 0;
        feedback.ResetCounter(securityTradeViewModelArray.Length);
        foreach (SecurityTradeViewModel info in securityTradeViewModelArray)
        {
          if (feedback.Cancel)
        ...
        if (num1 == 1)
        {
          int num2 = (int) Utils.Dialog((IWin32Window) this, "1 trade has been successfully deleted.", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
        }
        else
        {
          int num3 = (int) Utils.Dialog((IWin32Window) this, num1.ToString() + " trades have been successfully deleted.", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
        }
        return DialogResult.OK;
      }
      catch (Exception ex)
      {
        Tracing.Log(SecurityTradeListScreen.sw, nameof (SecurityTradeListScreen), TraceLevel.Error, "Error deleting trades: " + (object) ex);
    ...
    private DialogResult deleteTrade(SecurityTradeViewModel info, IProgressFeedback feedback)
    {
      try
      {
        feedback.Status = "Deleting trade '" + info.Name + "'...";
        SecurityTradeInfo trade = Session.SessionObjects.SecurityTradeManager.GetTrade(info.TradeID);
        if (trade == null)
          return DialogResult.OK;
        feedback.Details = "Deleting trade...";
        Session.SecurityTradeManager.DeleteTrade(trade.TradeID);
        return DialogResult.OK;
      }
      catch (Exception ex)
      {
        Tracing.Log(SecurityTradeListScreen.sw, nameof (SecurityTradeListScreen), TraceLevel.Error, "Error processing trade " + (object) info.TradeID + ": " + (object) ex);
    ...
    public bool isCurrentTrades() => this.cboFilterView.SelectedIndex == 0;

    private void btnArchive_Click(object sender, EventArgs e)
    {
      if (this.gvTradeList.SelectedItems.Count == 0)
      {
        int num1 = (int) Utils.Dialog((IWin32Window) this, "You must first select the trade(s) to be " + (this.isCurrentTrades() ? "archived" : "activated") + ".", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
      }
      else
      {
        if ((!this.isCurrentTrades() ? Utils.Dialog((IWin32Window) this, "Are you sure you want to activate the selected trade(s)?", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) : Utils.Dialog((IWin32Window) this, "Are you sure you want to archive the selected trade(s)?", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)) != DialogResult.Yes)
          return;
        ...
        try
        {
          ...
          foreach (GVItem selectedItem in this.gvTradeList.SelectedItems)
          {
            SecurityTradeViewModel tag = (SecurityTradeViewModel) selectedItem.Tag;
            if (tag.CompletionPercent < 100M && this.isCurrentTrades())
            {
              if (Utils.Dialog((IWin32Window) this, "Are you sure you want to archive incomplete trade ('" + tag.Name + "')?", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
                this.archiveActivateTrade(tag, this.isCurrentTrades());
            }
            else
              this.archiveActivateTrade(tag, this.isCurrentTrades());
          }
    ...
    private void archiveActivateTrade(SecurityTradeViewModel info, bool archive)
    {
      info.Archived = archive;
      if (archive)
        Session.SecurityTradeManager.SetTradeStatus(info.TradeID, TradeStatus.Archived);
      else
        Session.SecurityTradeManager.SetTradeStatus(info.TradeID, info.Status);
    }
    ...
    {
      using (ViewManagementDialog managementDialog = new ViewManagementDialog(EllieMae.EMLite.ClientServer.TemplateSettingsType.SecurityTradeView, false, "SecurityTrades.DefaultView"))
      {
    ...
    {
      SecurityTradeView securityTradeView = new SecurityTradeView(this.currentView.Name);
      securityTradeView.Layout = this.gvLayoutManager.GetCurrentLayout();
      securityTradeView.Filter = this.GetCurrentFilter();
      using (SaveViewTemplateDialog viewTemplateDialog = new SaveViewTemplateDialog(EllieMae.EMLite.ClientServer.TemplateSettingsType.SecurityTradeView, (object) securityTradeView, this.getViewNameList(), this.currentView.Name != this.standardViewName))
      {
        ...
        if (!viewTemplateDialog.SelectedEntry.Equals((object) this.fsViewEntry))
          this.updateCurrentView(securityTradeView, viewTemplateDialog.SelectedEntry);
      }
      this.currentView = securityTradeView;
      this.btnSaveView.Enabled = false;
      this.btnRefreshView.Enabled = false;
    }

    private void updateCurrentView(SecurityTradeView view, FileSystemEntry e)
    {
      this.suspendEvents = true;
    ...
    public void SetCurrentView(FileSystemEntry fsEntry)
    {
      try
      {
        ...
        {
          SecurityTradeView defaultTradeView = this.getDefaultTradeView();
          this.fsViewEntry = fsEntry;
          this.setCurrentView(defaultTradeView);
        }
        else
        {
          SecurityTradeView templateSettings = (SecurityTradeView) Session.ConfigurationManager.GetTemplateSettings(EllieMae.EMLite.ClientServer.TemplateSettingsType.SecurityTradeView, fsEntry);
      ...
      {
        Tracing.Log(SecurityTradeListScreen.sw, nameof (SecurityTradeListScreen), TraceLevel.Error, "Error opening view: " + (object) ex);
        ErrorDialog.Display(ex);
      }
    }

    private void setCurrentView(SecurityTradeView view)
    {
      this.currentView = view;
      this.suspendEvents = true;
    ...
    private SecurityTradeView getDefaultTradeView()
    {
      SecurityTradeView defaultTradeView = new SecurityTradeView(this.standardViewName);
      TableLayout tableLayout = new TableLayout();
      tableLayout.AddColumn(new TableLayout.Column("SecurityTradeDetails.CommitmentDate", "Commitment Date", HorizontalAlignment.Left, 100));
      tableLayout.AddColumn(new TableLayout.Column("SecurityTradeDetails.Name", "Security ID", HorizontalAlignment.Left, 120));
      tableLayout.AddColumn(new TableLayout.Column("SecurityTradeDetails.SecurityType", "Security Type", HorizontalAlignment.Left, 110));
      tableLayout.AddColumn(new TableLayout.Column("SecurityTradeDetails.Coupon", "Coupon", HorizontalAlignment.Right, 110));
      tableLayout.AddColumn(new TableLayout.Column("SecurityTradeDetails.SettlementDate", "Settlement Date", HorizontalAlignment.Left, 100));
      tableLayout.AddColumn(new TableLayout.Column("SecurityTradeDetails.DealerName", "Dealer", HorizontalAlignment.Left, 120));
      tableLayout.AddColumn(new TableLayout.Column("SecurityTradeDetails.TradeAmount", "Trade Amount", HorizontalAlignment.Right, 110));
      tableLayout.AddColumn(new TableLayout.Column("SecurityTradeDetails.MinAmount", "Min. Amount", HorizontalAlignment.Right, 110));
      tableLayout.AddColumn(new TableLayout.Column("SecurityTradeDetails.MaxAmount", "Max. Amount", HorizontalAlignment.Right, 110));
      tableLayout.AddColumn(new TableLayout.Column("SecurityTradeSummary.TotalAmount", "Assigned Loan Trade Amount", HorizontalAlignment.Right, 110));
      tableLayout.AddColumn(new TableLayout.Column("SecurityTradeSummary.CompletionPercent", "Completion Percent", HorizontalAlignment.Left, 110));
    ...
    private void exportAllLoansToExcel()
    {
      using (CursorActivator.Wait())
      {
        ...
        if (this.tradeCursor.GetItemCount() == 0)
          return;
        foreach (SecurityTradeViewModel tradeInfo in (SecurityTradeViewModel[]) this.tradeCursor.GetItems(0, this.tradeCursor.GetItemCount()))
        {
    ...
    private void gvTradeList_SelectedIndexChanged(object sender, EventArgs e)
    {
      ...
      this.btnMakeCurrent.Enabled = count > 0;
    ...
    private void InitializeComponent()
    {
      ...
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (SecurityTradeListScreen));
      ...
      this.cBoxView.AutoCompleteCustomSource.AddRange(new string[2]
      {
        "Current Trades",
        "Archived Trades"
      });
      ...
      this.lblFilter.Location = new Point(269, 8);
      ...
      this.label3.Location = new Point(229, 8);
      ...
      this.Name = nameof (SecurityTradeListScreen);
