// Decompiled with JetBrains decompiler
// Type: Elli.EncompassPlatform.Common.DataContracts.DocumentTemplateGetContract
// Assembly: ServiceInterface, Version=1.0.0.0, Culture=neutral, PublicKeyToken=d11ef57bba4acf91
// MVID: DF0C2B89-A027-4FA0-9669-4D2AA36A4D74
// Assembly location: C:\SmartClientCache\Apps\UAC\Ellie Mae\xIHR5EqGa7zPnRG0YpD5z4TPAB0=\Encompass360\ServiceInterface.dll

using System.Runtime.Serialization;

#nullable disable
namespace Elli.EncompassPlatform.Common.DataContracts
{
  [DataContract(Name = "DocumentTemplateGet", Namespace = "http://www.elliemae.com/encompass/platform")]
  public class DocumentTemplateGetContract
  {
    [DataMember]
    public string Guid { get; set; }

    [DataMember]
    public string Name { get; set; }

    [DataMember]
    public string Description { get; set; }

    [DataMember]
    public string Source { get; set; }

    [DataMember]
    public int DaysDue { get; set; }

    [DataMember]
    public int DaysTillExpire { get; set; }

    [DataMember]
    public bool OpeningDocumentIndicator { get; set; }

    [DataMember]
    public bool PreClosingDocumentIndicator { get; set; }

    [DataMember]
    public bool ClosingDocumentIndicator { get; set; }

    [DataMember]
    public bool IsWebCenterIndicator { get; set; }

    [DataMember]
    public bool IsTPOWebcenterPortalIndicator { get; set; }

    [DataMember]
    public bool IsThirdPartyDocIndicator { get; set; }

    [DataMember]
    public int SignatureType { get; set; }

    [DataMember]
    public int ConversionType { get; set; }

    [DataMember]
    public int SourceType { get; set; }

    [DataMember]
    public bool SaveOriginalFormat { get; set; }
  }
}
