...
// Type: EllieMae.EMLite.RemotingServices.AutomatedPurchaseConditionBpmManager
...
{
  public class AutomatedPurchaseConditionBpmManager : BpmManager
  {
    internal static AutomatedPurchaseConditionBpmManager Instance
    {
      get => Session.DefaultInstance.AutomatedPurchaseConditionsBpmManager;
    }
    ...
    internal AutomatedPurchaseConditionBpmManager(Sessions.Session session)
      : base(session, BizRuleType.MilestoneTemplateConditions, ClientSessionCacheID.BpmAutomatedPurchaseConditions)
    {
    }
      ...
      ConditionType conditionType)
    {
      ...
      {
        case "":
          AutomatedPurchaseCondition[][] purchaseConditionArray = new AutomatedPurchaseCondition[12][]
          {
          ...
          if (loanData.GetField("1811") != string.Empty)
          {
            ...
            purchaseConditionArray[9] = this.getConditions(BizRule.Condition.Occupancy, channelValue, (int) conditionState3, (string) null, (string) null);
          }
          else
            purchaseConditionArray[9] = (AutomatedPurchaseCondition[]) null;
          purchaseConditionArray[10] = this.getConditions(BizRule.Condition.Occupancy, channelValue, (int) conditionState1, (string) null, (string) null);
          purchaseConditionArray[11] = this.getAdvancedAutomatedPurchaseConditions(channelValue, loanData);
          List<string> stringList = new List<string>();
          for (int index = 0; index < purchaseConditionArray.Length; ++index)
          {
            if (purchaseConditionArray[index] != null && purchaseConditionArray[index].Length != 0)
            {
              foreach (AutomatedPurchaseCondition purchaseCondition in purchaseConditionArray[index])
              {
                if (purchaseCondition.ConditionType == conditionType && !stringList.Contains(purchaseCondition.ConditionName))
                  stringList.Add(purchaseCondition.ConditionName);
              }
            }
          }
    ...
    private AutomatedPurchaseCondition[] getConditions(
      ...
      string conditionState2)
    {
      ...
      if (activeRule == null)
        return new AutomatedPurchaseCondition[0];
      AutomatedPurchaseConditionRuleInfo[] conditionRuleInfoArray = new AutomatedPurchaseConditionRuleInfo[activeRule.Length];
      for (int index = 0; index < activeRule.Length; ++index)
        conditionRuleInfoArray[index] = (AutomatedPurchaseConditionRuleInfo) activeRule[index];
      List<AutomatedPurchaseCondition> purchaseConditionList = new List<AutomatedPurchaseCondition>();
      for (int index = 0; index < conditionRuleInfoArray.Length; ++index)
        purchaseConditionList.AddRange((IEnumerable<AutomatedPurchaseCondition>) conditionRuleInfoArray[index].Conditions);
      return purchaseConditionList.ToArray();
    }

    private AutomatedPurchaseCondition[] getAdvancedAutomatedPurchaseConditions(
      string channelValue,
      LoanData loan)
    {
      List<AutomatedPurchaseCondition> purchaseConditionList = new List<AutomatedPurchaseCondition>();
      ...
      foreach (ConditionEvaluator conditionEvaluator in this.GetConditionEvaluators())
      {
        if (conditionEvaluator.Rule.Condition == BizRule.Condition.AdvancedCoding && conditionEvaluator.AppliesTo(context) && conditionEvaluator.Rule.Condition2.IndexOf(channelValue) > -1)
          purchaseConditionList.AddRange((IEnumerable<AutomatedPurchaseCondition>) ((AutomatedPurchaseConditionRuleInfo) conditionEvaluator.Rule).Conditions);
      }
      return purchaseConditionList.ToArray();
    }
  }
