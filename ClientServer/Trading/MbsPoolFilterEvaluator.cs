...
// Type: EllieMae.EMLite.Trading.MbsPoolFilterEvaluator
...
{
  public class MbsPoolFilterEvaluator(TradeFilter filter) : TradeFilterEvaluator(filter)
  {
    public override bool Evaluate(
      ...
      out string errMsg)
    {
      MbsPoolViewModel mbsPoolViewModel = (MbsPoolViewModel) tradeObj;
      errMsg = "One or more loans assigned to this MBS Pool do not meet the eligibility criteria. Do you want to continue to save this pool?";
      ...
      if (this.Filter.FilterType == TradeFilterType.Simple && this.Filter.GetSimpleFilter().Milestones.Count > 0)
      {
        ...
        {
          errMsg = "The MBS Pool has one or more loans assigned to it for which the milestone has changed or been removed. Do you want to continue to save this pool?";
          return false;
        }
      ...
      {
        errMsg = "The MBS Pool cannot be saved due to loan(s) having been withdrawn. You will need to go to the Loans tab and delete the highlighted loan prior to any other updates.";
        return false;
      }
      if (mbsPoolViewModel != null && pinfo.IsRejectedByInvestor(mbsPoolViewModel.InvestorName))
        return false;
      errMsg = string.Empty;
      return true;
    }
    ...
    {
      MbsPoolInfo mbsPoolInfo = (MbsPoolInfo) tradeObj;
      ...
      if ((filterOption == EligibleLoanFilterOption.Rejected || filterOption == EligibleLoanFilterOption.All) && mbsPoolInfo != null && (mbsPoolInfo.InvestorName ?? "") != "")
        queryCriterion2 = queryCriterion2.And((QueryCriterion) new XRefValueCriterion("Loan.Guid", "TradeRejections.LoanGuid", (QueryCriterion) new StringValueCriterion("TradeRejections.Investor", mbsPoolInfo.InvestorName, StringMatchType.CaseInsensitive), false));
