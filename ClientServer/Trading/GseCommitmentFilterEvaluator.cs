...
// Type: EllieMae.EMLite.Trading.GseCommitmentFilterEvaluator
...
{
  public class GseCommitmentFilterEvaluator(TradeFilter filter) : 
    ...
    IFilterEvaluator
  {
      ...
      out string errMsg)
    {
      ...
      {
        errMsg = "The GSE Commitment cannot be saved due to loan(s) having been withdrawn. You will need to go to the Loans tab and delete the highlighted loan prior to any other updates.";
        return false;
      }
    ...
    {
      GSECommitmentInfo gseCommitmentInfo = (GSECommitmentInfo) tradeObj;
      ...
      if ((filterOption == EligibleLoanFilterOption.Rejected || filterOption == EligibleLoanFilterOption.All) && gseCommitmentInfo != null && (gseCommitmentInfo.InvestorName ?? "") != "")
        queryCriterion2 = queryCriterion2.And((QueryCriterion) new XRefValueCriterion("Loan.Guid", "TradeRejections.LoanGuid", (QueryCriterion) new StringValueCriterion("TradeRejections.Investor", gseCommitmentInfo.InvestorName, StringMatchType.CaseInsensitive), false));
