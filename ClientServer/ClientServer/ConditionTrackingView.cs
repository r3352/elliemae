...
// Type: EllieMae.EMLite.ClientServer.ConditionTrackingView
...
{
  [Serializable]
  public class ConditionTrackingView : BinaryConvertible<ConditionTrackingView>, ITemplateSetting
  {
    private string name;
    private FieldFilterList filter;
    private TableLayout layout;

    public ConditionTrackingView()
    {
    }

    public ConditionTrackingView(string name) => this.name = name;

    public ConditionTrackingView(XmlSerializationInfo info)
    {
    ...
    public static explicit operator ConditionTrackingView(BinaryObject binaryObject)
    {
      return BinaryConvertible<ConditionTrackingView>.Parse(binaryObject);
    }
  }
}
