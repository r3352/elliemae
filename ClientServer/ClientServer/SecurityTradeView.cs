...
// Type: EllieMae.EMLite.ClientServer.SecurityTradeView
...
{
  [Serializable]
  public class SecurityTradeView : BinaryConvertible<SecurityTradeView>, ITemplateSetting
  {
    private string name;
    private FieldFilterList filter;
    private TableLayout layout;

    public SecurityTradeView()
    {
    }

    public SecurityTradeView(string name) => this.name = name;

    public SecurityTradeView(XmlSerializationInfo info)
    {
    ...
    public static explicit operator SecurityTradeView(BinaryObject binaryObject)
    {
      return BinaryConvertible<SecurityTradeView>.Parse(binaryObject);
    }
