...
// Type: EllieMae.EMLite.Common.UI.ReportFieldDefinitions.TradeAssignedMbsPoolFieldDefs
...
{
  public class TradeAssignedMbsPoolFieldDefs : ReportFieldDefs
  {
    private TradeAssignedMbsPoolFieldDefs(Sessions.Session session)
      : base(session)
    {
      foreach (ReportFieldDef fieldDef in (ReportFieldDefContainer) MbsPoolReportFieldDefs.GetFieldDefs())
        this.Add(fieldDef);
    }
    ...
    public static TradeAssignedMbsPoolFieldDefs GetFieldDefs()
    {
      return new TradeAssignedMbsPoolFieldDefs(Session.DefaultInstance);
    }
  }
}
