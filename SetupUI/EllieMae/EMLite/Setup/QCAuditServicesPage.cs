...
// Type: EllieMae.EMLite.Setup.QCAuditServicesPage
...
{
  public class QCAuditServicesPage : Form
  {
    private static string investorCategory = "QCAudit";
    ...
    private Dictionary<string, QCAuditUI> investors = new Dictionary<string, QCAuditUI>();
    ...
    public static string InvestorCategory => QCAuditServicesPage.investorCategory;

    public QCAuditServicesPage(
    ...
    private void selectAll(bool custom)
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        ...
        if (custom)
        {
          ((QCAuditUI) listViewItem.Tag).InvestorServiceAclInfo.CustomAccess = AclResourceAccess.ReadWrite;
          listViewItem.ImageIndex = 0;
        }
        else
          ((QCAuditUI) listViewItem.Tag).InvestorServiceAclInfo.PersonaAccess = AclResourceAccess.ReadWrite;
      }
      this.dirty = true;
    }
    ...
    private void deselectAll(bool custom)
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        ...
        if (custom)
        {
          ((QCAuditUI) listViewItem.Tag).InvestorServiceAclInfo.CustomAccess = AclResourceAccess.ReadOnly;
          listViewItem.ImageIndex = 0;
        }
        else
          ((QCAuditUI) listViewItem.Tag).InvestorServiceAclInfo.PersonaAccess = AclResourceAccess.ReadOnly;
      }
      this.dirty = true;
    }
    ...
    {
      this.investors = new Dictionary<string, QCAuditUI>();
      int num = 0;
      foreach (PartnerResponseBody partnerResponseBody in new DataDocsServiceHelper(this.session).GetInvestorsList().Where<PartnerResponseBody>((Func<PartnerResponseBody, bool>) (provider => provider.Category.Equals(QCAuditServicesPage.investorCategory, StringComparison.InvariantCultureIgnoreCase))))
      {
        string str = partnerResponseBody.PartnerID + partnerResponseBody.ProductName;
        if (!this.investors.ContainsKey(str))
        {
          InvestorServiceAclInfo investorServiceAclInfo = new InvestorServiceAclInfo(str, 9014, QCAuditServicesPage.investorCategory);
          QCAuditUI qcAuditUi = new QCAuditUI(investorServiceAclInfo, partnerResponseBody.DisplayName, ++num);
          this.availableServices.Add(investorServiceAclInfo);
          this.investors.Add(str, qcAuditUi);
        }
      }
    }
    ...
    private void loadInvestorServicesForPersona()
    {
      this.initial = true;
      this.dirty = false;
      InvestorServiceAclInfo.InvestorServicesDefaultSetting servicesDefaultSetting = this.investorServiceMgr.GetInvestorServicesDefaultSetting(this.feature, this.personaID, QCAuditServicesPage.investorCategory);
      this.previousSettings = servicesDefaultSetting;
      InvestorServiceAclInfo[] permissions = this.investorServiceMgr.GetPermissions(this.feature, this.personaID, QCAuditServicesPage.investorCategory, this.availableServices.ToArray());
      this.lsvInvestors.Items.Clear();
      if (this.investors != null && this.investors.Count > 0)
        this.lsvInvestors.ListViewItemSorter = (IComparer) this.investors.Values.First<QCAuditUI>();
      foreach (InvestorServiceAclInfo investorServiceAclInfo in permissions)
      {
        QCAuditUI qcAuditUi = (QCAuditUI) null;
        this.investors.TryGetValue(investorServiceAclInfo.ProviderCompanyCode, out qcAuditUi);
        if (qcAuditUi != null)
        {
          qcAuditUi.InvestorServiceAclInfo = investorServiceAclInfo;
          ListViewItem listViewItem = new ListViewItem(qcAuditUi.DisplayName);
          listViewItem.Tag = (object) qcAuditUi;
    ...
    private void loadInvestorServicesForUser()
    {
      this.initial = true;
      this.dirty = false;
      InvestorServiceAclInfo.InvestorServicesDefaultSetting servicesDefaultSetting = this.investorServiceMgr.GetUserSpecificInvestorServicesDefaultSetting(this.feature, this.userID, QCAuditServicesPage.investorCategory, this.personaList);
      InvestorServiceAclInfo[] permissions = this.investorServiceMgr.GetPermissions(this.feature, this.userID, QCAuditServicesPage.investorCategory, this.personaList, this.availableServices.ToArray());
      this.lsvInvestors.Items.Clear();
      if (this.investors != null && this.investors.Count > 0)
        this.lsvInvestors.ListViewItemSorter = (IComparer) this.investors.Values.First<QCAuditUI>();
      foreach (InvestorServiceAclInfo investorServiceAclInfo in permissions)
      ...
      if (servicesDefaultSetting == InvestorServiceAclInfo.InvestorServicesDefaultSetting.NotSpecified)
      {
        ...
        servicesDefaultSetting = this.investorServiceMgr.GetInvestorServicesDefaultSetting(this.feature, this.userID, QCAuditServicesPage.investorCategory, this.personaList);
      }
      else
    ...
    public void SaveData()
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        InvestorServiceAclInfo investorServiceAclInfo = ((QCAuditUI) listViewItem.Tag).InvestorServiceAclInfo;
      ...
      if (!this.isPersonal)
      {
        this.investorServiceMgr.SetPermissions(this.feature, investorServiceAclInfoList.ToArray(), this.personaID, QCAuditServicesPage.investorCategory, defaultValue);
        this.investorServiceMgr.SetDefaultValue(this.feature, this.personaID, QCAuditServicesPage.investorCategory, defaultValue);
      }
      else
      {
        if (this.lblDefaultSetting.ImageIndex == 1)
          defaultValue = InvestorServiceAclInfo.InvestorServicesDefaultSetting.NotSpecified;
        this.investorServiceMgr.SetPermissions(this.feature, investorServiceAclInfoList.ToArray(), this.userID, QCAuditServicesPage.investorCategory, defaultValue);
        this.investorServiceMgr.SetDefaultValue(this.feature, this.userID, QCAuditServicesPage.investorCategory, defaultValue);
      }
      this.dirty = false;
    }
    ...
    private void lsvInvestors_ItemChecked(object sender, ItemCheckedEventArgs e)
    {
      if (this.initial || this.readOnly)
        return;
      this.dirty = true;
      QCAuditUI tag = (QCAuditUI) e.Item.Tag;
    ...
    private void miLinkTo_Click(object sender, EventArgs e)
    {
      ...
      {
        QCAuditUI tag = (QCAuditUI) selectedItem.Tag;
    ...
    private void lsvInvestors_ItemCheck(object sender, ItemCheckEventArgs e)
    {
      if (this.initial)
        return;
      InvestorServiceAclInfo investorServiceAclInfo = ((QCAuditUI) this.lsvInvestors.Items[e.Index].Tag).InvestorServiceAclInfo;
      if (this.readOnly)
      {
        this.dirty = false;
    ...
    private void miDefaultLinkTo_Click(object sender, EventArgs e)
    {
      this.dirty = true;
      this.initial = true;
      switch (this.investorServiceMgr.GetPersonasInvestorServicesDefaultSetting(this.feature, this.userID, QCAuditServicesPage.investorCategory, this.personaList))
      {
        case InvestorServiceAclInfo.InvestorServicesDefaultSetting.None:
          this.rdbNone.Checked = true;
          break;
      ...
      {
        listViewItem.ImageIndex = 1;
        QCAuditUI tag = (QCAuditUI) listViewItem.Tag;
        InvestorServiceAclInfo investorServiceAclInfo = tag.InvestorServiceAclInfo;
        if (investorServiceAclInfo != null)
          listViewItem.Checked = investorServiceAclInfo.PersonaAccess == AclResourceAccess.ReadWrite;
        investorServiceAclInfo.CustomAccess = AclResourceAccess.None;
    ...
    private void InitializeComponent()
    {
      ...
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (QCAuditServicesPage));
      ...
      this.Name = "QCAuditPage";
      this.ShowIcon = false;
      this.ShowInTaskbar = false;
      this.StartPosition = FormStartPosition.CenterParent;
      this.Text = "Select QC Audit Services Firm";
