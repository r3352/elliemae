...
// Type: EllieMae.EMLite.Setup.WholesaleLenderServicePage
...
{
  public class WholesaleLenderServicePage : Form
  {
    private static string investorCategory = "WholesaleLender";
    ...
    private Dictionary<string, WholesaleLenderUI> investors = new Dictionary<string, WholesaleLenderUI>();
    ...
    public static string InvestorCategory => WholesaleLenderServicePage.investorCategory;

    public WholesaleLenderServicePage(
    ...
    public WholesaleLenderServicePage(
      ...
      bool readOnly,
      int option)
      : this(session, feature, readOnly, option)
    {
    ...
    private void selectAll(bool custom)
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        ...
        if (custom)
        {
          ((WholesaleLenderUI) listViewItem.Tag).InvestorServiceAclInfo.CustomAccess = AclResourceAccess.ReadWrite;
          listViewItem.ImageIndex = 0;
        }
        else
          ((WholesaleLenderUI) listViewItem.Tag).InvestorServiceAclInfo.PersonaAccess = AclResourceAccess.ReadWrite;
      }
      this.dirty = true;
    }
    ...
    private void deselectAll(bool custom)
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        ...
        if (custom)
        {
          ((WholesaleLenderUI) listViewItem.Tag).InvestorServiceAclInfo.CustomAccess = AclResourceAccess.ReadOnly;
          listViewItem.ImageIndex = 0;
        }
        else
          ((WholesaleLenderUI) listViewItem.Tag).InvestorServiceAclInfo.PersonaAccess = AclResourceAccess.ReadOnly;
      }
      this.dirty = true;
    }
    ...
    {
      this.investors = new Dictionary<string, WholesaleLenderUI>();
      int num = 0;
      foreach (PartnerResponseBody partnerResponseBody in new DataDocsServiceHelper(this.session).GetInvestorsList().Where<PartnerResponseBody>((Func<PartnerResponseBody, bool>) (provider => provider.Category.Equals(WholesaleLenderServicePage.investorCategory, StringComparison.InvariantCultureIgnoreCase))))
      {
        string str = partnerResponseBody.PartnerID + partnerResponseBody.ProductName;
        if (!this.investors.ContainsKey(str))
        {
          InvestorServiceAclInfo investorServiceAclInfo = new InvestorServiceAclInfo(str, 9014, WholesaleLenderServicePage.investorCategory);
          WholesaleLenderUI wholesaleLenderUi = new WholesaleLenderUI(investorServiceAclInfo, partnerResponseBody.DisplayName, ++num);
          this.availableServices.Add(investorServiceAclInfo);
          this.investors.Add(str, wholesaleLenderUi);
        }
      }
    }
    ...
    private void loadInvestorServicesForPersona()
    {
      this.initial = true;
      this.dirty = false;
      InvestorServiceAclInfo.InvestorServicesDefaultSetting servicesDefaultSetting = this.investorServiceMgr.GetInvestorServicesDefaultSetting(this.feature, this.personaID, WholesaleLenderServicePage.investorCategory);
      this.previousSettings = servicesDefaultSetting;
      InvestorServiceAclInfo[] permissions = this.investorServiceMgr.GetPermissions(this.feature, this.personaID, WholesaleLenderServicePage.investorCategory, this.availableServices.ToArray());
      this.lsvInvestors.Items.Clear();
      if (this.investors != null && this.investors.Count > 0)
        this.lsvInvestors.ListViewItemSorter = (IComparer) this.investors.Values.First<WholesaleLenderUI>();
      foreach (InvestorServiceAclInfo investorServiceAclInfo in permissions)
      {
        WholesaleLenderUI wholesaleLenderUi = (WholesaleLenderUI) null;
        this.investors.TryGetValue(investorServiceAclInfo.ProviderCompanyCode, out wholesaleLenderUi);
        if (wholesaleLenderUi != null)
        {
          wholesaleLenderUi.InvestorServiceAclInfo = investorServiceAclInfo;
          ListViewItem listViewItem = new ListViewItem(wholesaleLenderUi.DisplayName);
          listViewItem.Tag = (object) wholesaleLenderUi;
    ...
    private void loadInvestorServicesForUser()
    {
      this.initial = true;
      this.dirty = false;
      InvestorServiceAclInfo.InvestorServicesDefaultSetting servicesDefaultSetting = this.investorServiceMgr.GetUserSpecificInvestorServicesDefaultSetting(this.feature, this.userID, WholesaleLenderServicePage.investorCategory, this.personaList);
      InvestorServiceAclInfo[] permissions = this.investorServiceMgr.GetPermissions(this.feature, this.userID, WholesaleLenderServicePage.investorCategory, this.personaList, this.availableServices.ToArray());
      this.lsvInvestors.Items.Clear();
      if (this.investors != null && this.investors.Count > 0)
        this.lsvInvestors.ListViewItemSorter = (IComparer) this.investors.Values.First<WholesaleLenderUI>();
      foreach (InvestorServiceAclInfo investorServiceAclInfo in permissions)
      ...
      if (servicesDefaultSetting == InvestorServiceAclInfo.InvestorServicesDefaultSetting.NotSpecified)
      {
        ...
        servicesDefaultSetting = this.investorServiceMgr.GetInvestorServicesDefaultSetting(this.feature, this.userID, WholesaleLenderServicePage.investorCategory, this.personaList);
      }
      else
    ...
    public void SaveData()
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        InvestorServiceAclInfo investorServiceAclInfo = ((WholesaleLenderUI) listViewItem.Tag).InvestorServiceAclInfo;
      ...
      if (!this.isPersonal)
      {
        this.investorServiceMgr.SetPermissions(this.feature, investorServiceAclInfoList.ToArray(), this.personaID, WholesaleLenderServicePage.investorCategory, defaultValue);
        this.investorServiceMgr.SetDefaultValue(this.feature, this.personaID, WholesaleLenderServicePage.investorCategory, defaultValue);
      }
      else
      {
        if (this.lblDefaultSetting.ImageIndex == 1)
          defaultValue = InvestorServiceAclInfo.InvestorServicesDefaultSetting.NotSpecified;
        this.investorServiceMgr.SetPermissions(this.feature, investorServiceAclInfoList.ToArray(), this.userID, WholesaleLenderServicePage.investorCategory, defaultValue);
        this.investorServiceMgr.SetDefaultValue(this.feature, this.userID, WholesaleLenderServicePage.investorCategory, defaultValue);
      }
      this.dirty = false;
    }
    ...
    private void lsvInvestors_ItemChecked(object sender, ItemCheckedEventArgs e)
    {
      if (this.initial || this.readOnly)
        return;
      this.dirty = true;
      WholesaleLenderUI tag = (WholesaleLenderUI) e.Item.Tag;
    ...
    private void miLinkTo_Click(object sender, EventArgs e)
    {
      ...
      {
        WholesaleLenderUI tag = (WholesaleLenderUI) selectedItem.Tag;
    ...
    private void lsvInvestors_ItemCheck(object sender, ItemCheckEventArgs e)
    {
      if (this.initial)
        return;
      InvestorServiceAclInfo investorServiceAclInfo = ((WholesaleLenderUI) this.lsvInvestors.Items[e.Index].Tag).InvestorServiceAclInfo;
      if (this.readOnly)
      {
        this.dirty = false;
    ...
    private void miDefaultLinkTo_Click(object sender, EventArgs e)
    {
      this.dirty = true;
      this.initial = true;
      switch (this.investorServiceMgr.GetPersonasInvestorServicesDefaultSetting(this.feature, this.userID, WholesaleLenderServicePage.investorCategory, this.personaList))
      {
        case InvestorServiceAclInfo.InvestorServicesDefaultSetting.None:
          this.rdbNone.Checked = true;
          break;
      ...
      {
        listViewItem.ImageIndex = 1;
        WholesaleLenderUI tag = (WholesaleLenderUI) listViewItem.Tag;
        InvestorServiceAclInfo investorServiceAclInfo = tag.InvestorServiceAclInfo;
        if (investorServiceAclInfo != null)
          listViewItem.Checked = investorServiceAclInfo.PersonaAccess == AclResourceAccess.ReadWrite;
        investorServiceAclInfo.CustomAccess = AclResourceAccess.None;
    ...
    private void InitializeComponent()
    {
      ...
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (WholesaleLenderServicePage));
      ...
      this.Name = nameof (WholesaleLenderServicePage);
      this.ShowIcon = false;
      this.ShowInTaskbar = false;
      this.StartPosition = FormStartPosition.CenterParent;
      this.Text = "Select Wholesale Lenders";
