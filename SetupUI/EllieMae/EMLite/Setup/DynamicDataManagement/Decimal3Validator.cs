...
// Type: EllieMae.EMLite.Setup.DynamicDataManagement.Decimal3Validator
...
{
  public class Decimal3Validator : IValidator
  {
      ...
      string[] options)
    {
      ...
      try
      {
        if (cellData.Criteria == DDMCriteria.Range)
        {
          ...
          Utils.FormatInput(strArray[0], FieldFormat.DECIMAL_3, ref needsUpdate);
          if (needsUpdate)
            throw new Exception("Format mismatch");
          Utils.FormatInput(strArray[1], FieldFormat.DECIMAL_3, ref needsUpdate);
        ...
        else if (cellData.Criteria == DDMCriteria.ListOfValues)
        {
          ...
          {
            Utils.FormatInput(orgval, FieldFormat.DECIMAL_3, ref needsUpdate);
            if (needsUpdate)
              throw new Exception("Format mismatch");
          }
        }
        else
          Utils.FormatInput(cellData.Data, FieldFormat.DECIMAL_3, ref needsUpdate);
        if (needsUpdate)
          throw new Exception("Format mismatch");
      }
      catch (Exception ex)
