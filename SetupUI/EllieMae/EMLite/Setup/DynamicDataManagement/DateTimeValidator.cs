...
// Type: EllieMae.EMLite.Setup.DynamicDataManagement.DateTimeValidator
      ...
      string[] options)
    {
      ...
      try
      {
        ...
        if (cellData.Criteria == DDMCriteria.Range)
        {
          ...
          Utils.FormatInput(Convert.ToDateTime(strArray[0]).ToString("MM/dd/yyyy hh:mm:ss"), FieldFormat.DATETIME, ref needsUpdate);
          if (needsUpdate)
          ...
          Utils.FormatInput(Convert.ToDateTime(strArray[1]).ToString("MM/dd/yyyy hh:mm:ss"), FieldFormat.DATETIME, ref needsUpdate);
          if (!this.CheckBoundaryCondition(Convert.ToDateTime(strArray[0])))
            throw new Exception("Value provided is outside the range 01/01/1900 - 12/31/2199");
        ...
        else if (cellData.Criteria == DDMCriteria.ListOfValues)
        {
          ...
          {
            Utils.FormatInput(Convert.ToDateTime(str).ToString("MM/dd/yyyy hh:mm:ss"), FieldFormat.DATETIME, ref needsUpdate);
            if (needsUpdate)
              throw new Exception("Format mismatch");
            if (!this.CheckBoundaryCondition(Convert.ToDateTime(str)))
              throw new Exception("Value provided is outside the range 01/01/1900 - 12/31/2199");
          }
        }
        else
        {
          Utils.FormatInput(Convert.ToDateTime(cellData.Data).ToString("MM/dd/yyyy hh:mm:ss"), FieldFormat.DATETIME, ref needsUpdate);
