...
// Type: EllieMae.EMLite.Setup.DynamicDataManagement.MonthDayValidator
...
{
  public class MonthDayValidator : IValidator
  {
      ...
      string[] options)
    {
      ...
      try
      {
        if (cellData.Criteria == DDMCriteria.Range)
        {
          ...
          Utils.FormatInput(strArray[0], FieldFormat.MONTHDAY, ref needsUpdate);
          if (needsUpdate)
            throw new Exception("Format mismatch");
          Utils.FormatInput(strArray[1], FieldFormat.MONTHDAY, ref needsUpdate);
        ...
        else if (cellData.Criteria == DDMCriteria.ListOfValues)
        {
          ...
          {
            Utils.FormatInput(orgval, FieldFormat.MONTHDAY, ref needsUpdate);
            if (needsUpdate)
              throw new Exception("Format mismatch");
          }
        }
        else
          Utils.FormatInput(cellData.Data, FieldFormat.MONTHDAY, ref needsUpdate);
        if (needsUpdate)
          throw new Exception("Format mismatch");
      }
      catch (Exception ex)
