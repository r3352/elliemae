...
// Type: EllieMae.EMLite.Setup.DynamicDataManagement.Decimal5Validator
...
{
  public class Decimal5Validator : IValidator
  {
      ...
      string[] options)
    {
      ...
      try
      {
        if (cellData.Criteria == DDMCriteria.Range)
        {
          ...
          Utils.FormatInput(strArray[0], FieldFormat.DECIMAL_5, ref needsUpdate);
          if (needsUpdate)
            throw new Exception("Format mismatch");
          Utils.FormatInput(strArray[1], FieldFormat.DECIMAL_5, ref needsUpdate);
        ...
        else if (cellData.Criteria == DDMCriteria.ListOfValues)
        {
          ...
          {
            Utils.FormatInput(orgval, FieldFormat.DECIMAL_5, ref needsUpdate);
            if (needsUpdate)
              throw new Exception("Format mismatch");
          }
        }
        else
          Utils.FormatInput(cellData.Data, FieldFormat.DECIMAL_5, ref needsUpdate);
        if (needsUpdate)
          throw new Exception("Format mismatch");
      }
      catch (Exception ex)
