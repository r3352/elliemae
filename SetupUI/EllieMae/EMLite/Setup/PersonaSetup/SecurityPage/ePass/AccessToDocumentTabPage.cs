// Decompiled with JetBrains decompiler
// Type: EllieMae.EMLite.Setup.PersonaSetup.SecurityPage.ePass.AccessToDocumentTabPage
// Assembly: SetupUI, Version=24.3.0.5, Culture=neutral, PublicKeyToken=null
// MVID: B5055D69-6F9D-458D-8A91-C87166D995EA
// Assembly location: C:\SmartClientCache\Apps\UAC\Ellie Mae\xIHR5EqGa7zPnRG0YpD5z4TPAB0=\Encompass360\SetupUI.dll

using EllieMae.EMLite.ClientServer;
using EllieMae.EMLite.Common;
using EllieMae.EMLite.RemotingServices;
using EllieMae.EMLite.Setup.PersonaSetup.FeatureSecurity.ePass;
using System;
using System.ComponentModel;
using System.Drawing;
using System.Windows.Forms;

#nullable disable
namespace EllieMae.EMLite.Setup.PersonaSetup.SecurityPage.ePass
{
  public class AccessToDocumentTabPage : PersonaTreePageBase
  {
    private string accessToDocumentTabNodePath = "Access to Document Tab";
    private PipelineConfiguration pipelineConfiguration;
    private IContainer components;

    public event AccessToDocumentTabPage.FeatureStatusChanged FeatureStateChanged;

    public AccessToDocumentTabPage(
      Sessions.Session session,
      int personaId,
      EventHandler dirtyFlagChanged,
      PipelineConfiguration pipelineConfiguration)
      : base(session, dirtyFlagChanged)
    {
      this.bIsUserSetup = false;
      this.pipelineConfiguration = pipelineConfiguration;
      this.securityHelper = (IFeatureSecurityHelper) new ePassAccessToDocHelper(this.session, personaId);
      this.myInit();
      this.contextMenu1.MenuItems.Remove(this.menuItemLinkWithPersona);
      this.contextMenu1.MenuItems.Remove(this.menuItemDisconnectFromPersona);
    }

    public AccessToDocumentTabPage(
      Sessions.Session session,
      string userId,
      Persona[] personas,
      EventHandler dirtyFlagChanged,
      PipelineConfiguration pipelineConfiguration)
      : base(session, dirtyFlagChanged)
    {
      this.bIsUserSetup = true;
      this.pipelineConfiguration = pipelineConfiguration;
      this.securityHelper = (IFeatureSecurityHelper) new ePassAccessToDocHelper(this.session, userId, personas);
      this.myInit();
      this.treeViewTabs.ImageList = this.imgListTv;
      this.contextMenu1.MenuItems.Remove(this.menuItemCheckAll);
      this.contextMenu1.MenuItems.Remove(this.menuItemUncheckAll);
    }

    private void myInit()
    {
      this.InitializeComponent();
      this.Title = "Document Tabs";
      this.treeViewTabs.ShowLines = this.treeViewTabs.ShowRootLines = false;
      this.AfterCheckedEvent += new PersonaTreeNodeAfterChecked(this.AccessToDocumentTabPage_AfterCheckedEvent);
      if (this.pipelineConfiguration != null)
      {
        this.pipelineConfiguration.FeatureStateChanged += new PipelineConfiguration.PipelineConfigChanged(this.pipelineConfiguration_FeatureStateChanged);
        this.pipelineConfiguration_FeatureStateChanged(AclFeature.GlobalTab_Pipeline, this.pipelineConfiguration.HasPipelineLoanTabAccess() ? AclTriState.True : AclTriState.False, false);
      }
      this.init();
    }

    private void pipelineConfiguration_FeatureStateChanged(
      AclFeature feature,
      AclTriState state,
      bool gotoContact)
    {
      if (feature != AclFeature.GlobalTab_Pipeline)
        return;
      if (state == AclTriState.True)
      {
        this.treeViewTabs.Visible = true;
        this.setNoAccessLabel(false, AclFeature.GlobalTab_Pipeline);
      }
      else
      {
        this.treeViewTabs.Visible = false;
        this.setNoAccessLabel(true, AclFeature.GlobalTab_Pipeline);
      }
    }

    public void SaveData()
    {
      if (!this.NeedToSaveData())
        return;
      this.UpdatePermissions();
    }

    public void ResetData()
    {
      this.ResetTree();
      TreeNode treeNode = this.findTreeNode(this.accessToDocumentTabNodePath);
      if (treeNode == null)
        return;
      this.AccessToDocumentTabPage_AfterCheckedEvent(treeNode);
    }

    public bool NeedToSaveData() => this.hasBeenModified();

    public void GrandAccessToDocumentTab()
    {
      TreeNode treeNode = this.findTreeNode(this.accessToDocumentTabNodePath);
      if (treeNode == null || treeNode.Checked)
        return;
      treeNode.Checked = true;
    }

    public bool HasAccessToDocumentTab()
    {
      TreeNode treeNode = this.findTreeNode(this.accessToDocumentTabNodePath);
      return treeNode != null && treeNode.Checked;
    }

    private void AccessToDocumentTabPage_AfterCheckedEvent(TreeNode node)
    {
      if (this.securityHelper.NodeToFeature(node) != AclFeature.eFolder_AccessToDocumentTab || this.FeatureStateChanged == null)
        return;
      this.FeatureStateChanged(AclFeature.eFolder_AccessToDocumentTab, node.Checked ? AclTriState.True : AclTriState.False);
    }

    protected override void Dispose(bool disposing)
    {
      if (disposing && this.components != null)
        this.components.Dispose();
      base.Dispose(disposing);
    }

    private void InitializeComponent()
    {
      this.SuspendLayout();
      this.AutoScaleDimensions = new SizeF(6f, 13f);
      this.AutoScaleMode = AutoScaleMode.Font;
      this.BackColor = Color.WhiteSmoke;
      this.ClientSize = new Size(572, 65);
      this.Name = nameof (AccessToDocumentTabPage);
      this.ResumeLayout(false);
    }

    public delegate void FeatureStatusChanged(AclFeature feature, AclTriState state);
  }
}
