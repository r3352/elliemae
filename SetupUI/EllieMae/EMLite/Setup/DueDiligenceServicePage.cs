...
// Type: EllieMae.EMLite.Setup.DueDiligenceServicePage
...
{
  public class DueDiligenceServicePage : Form
  {
    private static string investorCategory = "DueDiligence";
    ...
    private Dictionary<string, DueDiligenceServiceUI> investors = new Dictionary<string, DueDiligenceServiceUI>();
    ...
    public static string InvestorCategory => DueDiligenceServicePage.investorCategory;

    public DueDiligenceServicePage(
    ...
    private void selectAll(bool custom)
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        ...
        if (custom)
        {
          ((DueDiligenceServiceUI) listViewItem.Tag).InvestorServiceAclInfo.CustomAccess = AclResourceAccess.ReadWrite;
          listViewItem.ImageIndex = 0;
        }
        else
          ((DueDiligenceServiceUI) listViewItem.Tag).InvestorServiceAclInfo.PersonaAccess = AclResourceAccess.ReadWrite;
      }
      this.dirty = true;
    }
    ...
    private void deselectAll(bool custom)
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        ...
        if (custom)
        {
          ((DueDiligenceServiceUI) listViewItem.Tag).InvestorServiceAclInfo.CustomAccess = AclResourceAccess.ReadOnly;
          listViewItem.ImageIndex = 0;
        }
        else
          ((DueDiligenceServiceUI) listViewItem.Tag).InvestorServiceAclInfo.PersonaAccess = AclResourceAccess.ReadOnly;
      }
      this.dirty = true;
    }
    ...
    {
      this.investors = new Dictionary<string, DueDiligenceServiceUI>();
      int num = 0;
      foreach (PartnerResponseBody partnerResponseBody in new DataDocsServiceHelper(this.session).GetInvestorsList().Where<PartnerResponseBody>((Func<PartnerResponseBody, bool>) (provider => provider.Category.Equals(DueDiligenceServicePage.investorCategory, StringComparison.InvariantCultureIgnoreCase))))
      {
        string str = partnerResponseBody.PartnerID + partnerResponseBody.ProductName;
        if (!this.investors.ContainsKey(str))
        {
          InvestorServiceAclInfo investorServiceAclInfo = new InvestorServiceAclInfo(str, 9010, DueDiligenceServicePage.investorCategory);
          DueDiligenceServiceUI diligenceServiceUi = new DueDiligenceServiceUI(investorServiceAclInfo, partnerResponseBody.DisplayName, ++num);
          this.availableServices.Add(investorServiceAclInfo);
          this.investors.Add(str, diligenceServiceUi);
        }
      }
    }
    ...
    private void loadInvestorServicesForPersona()
    {
      this.initial = true;
      this.dirty = false;
      InvestorServiceAclInfo.InvestorServicesDefaultSetting servicesDefaultSetting = this.investorServiceMgr.GetInvestorServicesDefaultSetting(this.feature, this.personaID, DueDiligenceServicePage.investorCategory);
      this.previousSettings = servicesDefaultSetting;
      InvestorServiceAclInfo[] permissions = this.investorServiceMgr.GetPermissions(this.feature, this.personaID, DueDiligenceServicePage.investorCategory, this.availableServices.ToArray());
      this.lsvInvestors.Items.Clear();
      if (this.investors != null && this.investors.Count > 0)
        this.lsvInvestors.ListViewItemSorter = (IComparer) this.investors.Values.First<DueDiligenceServiceUI>();
      foreach (InvestorServiceAclInfo investorServiceAclInfo in permissions)
      {
        DueDiligenceServiceUI diligenceServiceUi = (DueDiligenceServiceUI) null;
        this.investors.TryGetValue(investorServiceAclInfo.ProviderCompanyCode, out diligenceServiceUi);
        if (diligenceServiceUi != null)
        {
          diligenceServiceUi.InvestorServiceAclInfo = investorServiceAclInfo;
          ListViewItem listViewItem = new ListViewItem(diligenceServiceUi.DisplayName);
          listViewItem.Tag = (object) diligenceServiceUi;
    ...
    private void loadInvestorServicesForUser()
    {
      this.initial = true;
      this.dirty = false;
      InvestorServiceAclInfo.InvestorServicesDefaultSetting servicesDefaultSetting = this.investorServiceMgr.GetUserSpecificInvestorServicesDefaultSetting(this.feature, this.userID, DueDiligenceServicePage.investorCategory, this.personaList);
      InvestorServiceAclInfo[] permissions = this.investorServiceMgr.GetPermissions(this.feature, this.userID, DueDiligenceServicePage.investorCategory, this.personaList, this.availableServices.ToArray());
      this.lsvInvestors.Items.Clear();
      if (this.investors != null && this.investors.Count > 0)
        this.lsvInvestors.ListViewItemSorter = (IComparer) this.investors.Values.First<DueDiligenceServiceUI>();
      foreach (InvestorServiceAclInfo investorServiceAclInfo in permissions)
      ...
      if (servicesDefaultSetting == InvestorServiceAclInfo.InvestorServicesDefaultSetting.NotSpecified)
      {
        ...
        servicesDefaultSetting = this.investorServiceMgr.GetInvestorServicesDefaultSetting(this.feature, this.userID, DueDiligenceServicePage.investorCategory, this.personaList);
      }
      else
    ...
    public void SaveData()
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        InvestorServiceAclInfo investorServiceAclInfo = ((DueDiligenceServiceUI) listViewItem.Tag).InvestorServiceAclInfo;
      ...
      if (!this.isPersonal)
      {
        this.investorServiceMgr.SetPermissions(this.feature, investorServiceAclInfoList.ToArray(), this.personaID, DueDiligenceServicePage.investorCategory, defaultValue);
        this.investorServiceMgr.SetDefaultValue(this.feature, this.personaID, DueDiligenceServicePage.investorCategory, defaultValue);
      }
      else
      {
        if (this.lblDefaultSetting.ImageIndex == 1)
          defaultValue = InvestorServiceAclInfo.InvestorServicesDefaultSetting.NotSpecified;
        this.investorServiceMgr.SetPermissions(this.feature, investorServiceAclInfoList.ToArray(), this.userID, DueDiligenceServicePage.investorCategory, defaultValue);
        this.investorServiceMgr.SetDefaultValue(this.feature, this.userID, DueDiligenceServicePage.investorCategory, defaultValue);
      }
      this.dirty = false;
    }
    ...
    private void lsvInvestors_ItemChecked(object sender, ItemCheckedEventArgs e)
    {
      if (this.initial || this.readOnly)
        return;
      this.dirty = true;
      DueDiligenceServiceUI tag = (DueDiligenceServiceUI) e.Item.Tag;
    ...
    private void miLinkTo_Click(object sender, EventArgs e)
    {
      ...
      {
        DueDiligenceServiceUI tag = (DueDiligenceServiceUI) selectedItem.Tag;
    ...
    private void lsvInvestors_ItemCheck(object sender, ItemCheckEventArgs e)
    {
      if (this.initial)
        return;
      InvestorServiceAclInfo investorServiceAclInfo = ((DueDiligenceServiceUI) this.lsvInvestors.Items[e.Index].Tag).InvestorServiceAclInfo;
      if (this.readOnly)
      {
        this.dirty = false;
    ...
    private void miDefaultLinkTo_Click(object sender, EventArgs e)
    {
      this.dirty = true;
      this.initial = true;
      switch (this.investorServiceMgr.GetPersonasInvestorServicesDefaultSetting(this.feature, this.userID, DueDiligenceServicePage.investorCategory, this.personaList))
      {
        case InvestorServiceAclInfo.InvestorServicesDefaultSetting.None:
          this.rdbNone.Checked = true;
          break;
      ...
      {
        listViewItem.ImageIndex = 1;
        DueDiligenceServiceUI tag = (DueDiligenceServiceUI) listViewItem.Tag;
        InvestorServiceAclInfo investorServiceAclInfo = tag.InvestorServiceAclInfo;
        if (investorServiceAclInfo != null)
          listViewItem.Checked = investorServiceAclInfo.PersonaAccess == AclResourceAccess.ReadWrite;
        investorServiceAclInfo.CustomAccess = AclResourceAccess.None;
    ...
    private void InitializeComponent()
    {
      ...
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (DueDiligenceServicePage));
      ...
      this.Name = nameof (DueDiligenceServicePage);
      this.ShowIcon = false;
      this.ShowInTaskbar = false;
      this.StartPosition = FormStartPosition.CenterParent;
      this.Text = "Select Due Diligence Service";
