...
// Type: EllieMae.EMLite.Setup.SelectConditionForm
...
{
  public class SelectConditionForm : Form
  {
    private Sessions.Session session;
    private UnderwritingConditionTrackingSetup underwritingConditions;
    private PostClosingConditionTrackingSetup postConditions;
    ...
    public SelectConditionForm(Sessions.Session session)
    {
      this.session = session;
      this.InitializeComponent();
      this.cboConditionType.SelectedIndex = 0;
    ...
    private void cboConditionType_SelectedIndexChanged(object sender, EventArgs e)
    {
      ...
      switch (this.cboConditionType.Text)
      {
        case "Preliminary":
        case "Underwriting":
          if (this.underwritingConditions == null)
            this.underwritingConditions = (UnderwritingConditionTrackingSetup) this.session.ConfigurationManager.GetConditionTrackingSetup(ConditionType.Underwriting);
          this.reloadConditions((ConditionTrackingSetup) this.underwritingConditions);
          break;
        case "Post-Closing":
          if (this.postConditions == null)
            this.postConditions = (PostClosingConditionTrackingSetup) this.session.ConfigurationManager.GetConditionTrackingSetup(ConditionType.PostClosing);
          this.reloadConditions((ConditionTrackingSetup) this.postConditions);
          break;
      }
      Cursor.Current = Cursors.Default;
    }

    private void reloadConditions(ConditionTrackingSetup conditions)
    {
      ...
      if (conditions != null)
      {
        for (int index = 0; index < conditions.Count; ++index)
          this.gridViewConditions.Items.Add(new GVItem(conditions[index].Name)
          {
    ...
    public List<ConditionTemplate> GetSelectedConditions()
    {
      List<ConditionTemplate> selectedConditions = new List<ConditionTemplate>();
      for (int index = 0; index < this.gridViewConditions.SelectedItems.Count; ++index)
        selectedConditions.Add((ConditionTemplate) this.gridViewConditions.SelectedItems[index].Tag);
    ...
    private void InitializeComponent()
    {
      ...
      this.cboConditionType.Items.AddRange(new object[3]
      {
        (object) "Preliminary",
        (object) "Underwriting",
        (object) "Post-Closing"
      ...
      this.Name = nameof (SelectConditionForm);
