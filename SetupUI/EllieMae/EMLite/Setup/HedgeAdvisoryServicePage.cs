...
// Type: EllieMae.EMLite.Setup.HedgeAdvisoryServicePage
...
{
  public class HedgeAdvisoryServicePage : Form
  {
    private static string investorCategory = "HedgeAdvisory";
    ...
    private Dictionary<string, HedgeAdvisoryServiceUI> investors = new Dictionary<string, HedgeAdvisoryServiceUI>();
    ...
    public static string InvestorCategory => HedgeAdvisoryServicePage.investorCategory;

    public HedgeAdvisoryServicePage(
    ...
    private void selectAll(bool custom)
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        ...
        if (custom)
        {
          ((HedgeAdvisoryServiceUI) listViewItem.Tag).InvestorServiceAclInfo.CustomAccess = AclResourceAccess.ReadWrite;
          listViewItem.ImageIndex = 0;
        }
        else
          ((HedgeAdvisoryServiceUI) listViewItem.Tag).InvestorServiceAclInfo.PersonaAccess = AclResourceAccess.ReadWrite;
      }
      this.dirty = true;
    }
    ...
    private void deselectAll(bool custom)
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        ...
        if (custom)
        {
          ((HedgeAdvisoryServiceUI) listViewItem.Tag).InvestorServiceAclInfo.CustomAccess = AclResourceAccess.ReadOnly;
          listViewItem.ImageIndex = 0;
        }
        else
          ((HedgeAdvisoryServiceUI) listViewItem.Tag).InvestorServiceAclInfo.PersonaAccess = AclResourceAccess.ReadOnly;
      }
      this.dirty = true;
    }
    ...
    {
      this.investors = new Dictionary<string, HedgeAdvisoryServiceUI>();
      int num = 0;
      foreach (PartnerResponseBody partnerResponseBody in new DataDocsServiceHelper(this.session).GetInvestorsList().Where<PartnerResponseBody>((Func<PartnerResponseBody, bool>) (provider => provider.Category.Equals(HedgeAdvisoryServicePage.investorCategory, StringComparison.InvariantCultureIgnoreCase))))
      {
        string str = partnerResponseBody.PartnerID + partnerResponseBody.ProductName;
        if (!this.investors.ContainsKey(str))
        {
          InvestorServiceAclInfo investorServiceAclInfo = new InvestorServiceAclInfo(str, 9011, HedgeAdvisoryServicePage.investorCategory);
          HedgeAdvisoryServiceUI advisoryServiceUi = new HedgeAdvisoryServiceUI(investorServiceAclInfo, partnerResponseBody.DisplayName, ++num);
          this.availableServices.Add(investorServiceAclInfo);
          this.investors.Add(str, advisoryServiceUi);
        }
      }
    }
    ...
    private void loadInvestorServicesForPersona()
    {
      this.initial = true;
      this.dirty = false;
      InvestorServiceAclInfo.InvestorServicesDefaultSetting servicesDefaultSetting = this.investorServiceMgr.GetInvestorServicesDefaultSetting(this.feature, this.personaID, HedgeAdvisoryServicePage.investorCategory);
      this.previousSettings = servicesDefaultSetting;
      InvestorServiceAclInfo[] permissions = this.investorServiceMgr.GetPermissions(this.feature, this.personaID, HedgeAdvisoryServicePage.investorCategory, this.availableServices.ToArray());
      this.lsvInvestors.Items.Clear();
      if (this.investors != null && this.investors.Count > 0)
        this.lsvInvestors.ListViewItemSorter = (IComparer) this.investors.Values.First<HedgeAdvisoryServiceUI>();
      foreach (InvestorServiceAclInfo investorServiceAclInfo in permissions)
      {
        HedgeAdvisoryServiceUI advisoryServiceUi = (HedgeAdvisoryServiceUI) null;
        this.investors.TryGetValue(investorServiceAclInfo.ProviderCompanyCode, out advisoryServiceUi);
        if (advisoryServiceUi != null)
        {
          advisoryServiceUi.InvestorServiceAclInfo = investorServiceAclInfo;
          ListViewItem listViewItem = new ListViewItem(advisoryServiceUi.DisplayName);
          listViewItem.Tag = (object) advisoryServiceUi;
    ...
    private void loadInvestorServicesForUser()
    {
      this.initial = true;
      this.dirty = false;
      InvestorServiceAclInfo.InvestorServicesDefaultSetting servicesDefaultSetting = this.investorServiceMgr.GetUserSpecificInvestorServicesDefaultSetting(this.feature, this.userID, HedgeAdvisoryServicePage.investorCategory, this.personaList);
      InvestorServiceAclInfo[] permissions = this.investorServiceMgr.GetPermissions(this.feature, this.userID, HedgeAdvisoryServicePage.investorCategory, this.personaList, this.availableServices.ToArray());
      this.lsvInvestors.Items.Clear();
      if (this.investors != null && this.investors.Count > 0)
        this.lsvInvestors.ListViewItemSorter = (IComparer) this.investors.Values.First<HedgeAdvisoryServiceUI>();
      foreach (InvestorServiceAclInfo investorServiceAclInfo in permissions)
      ...
      if (servicesDefaultSetting == InvestorServiceAclInfo.InvestorServicesDefaultSetting.NotSpecified)
      {
        ...
        servicesDefaultSetting = this.investorServiceMgr.GetInvestorServicesDefaultSetting(this.feature, this.userID, HedgeAdvisoryServicePage.investorCategory, this.personaList);
      }
      else
    ...
    public void SaveData()
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        InvestorServiceAclInfo investorServiceAclInfo = ((HedgeAdvisoryServiceUI) listViewItem.Tag).InvestorServiceAclInfo;
      ...
      if (!this.isPersonal)
      {
        this.investorServiceMgr.SetPermissions(this.feature, investorServiceAclInfoList.ToArray(), this.personaID, HedgeAdvisoryServicePage.investorCategory, defaultValue);
        this.investorServiceMgr.SetDefaultValue(this.feature, this.personaID, HedgeAdvisoryServicePage.investorCategory, defaultValue);
      }
      else
      {
        if (this.lblDefaultSetting.ImageIndex == 1)
          defaultValue = InvestorServiceAclInfo.InvestorServicesDefaultSetting.NotSpecified;
        this.investorServiceMgr.SetPermissions(this.feature, investorServiceAclInfoList.ToArray(), this.userID, HedgeAdvisoryServicePage.investorCategory, defaultValue);
        this.investorServiceMgr.SetDefaultValue(this.feature, this.userID, HedgeAdvisoryServicePage.investorCategory, defaultValue);
      }
      this.dirty = false;
    }
    ...
    private void lsvInvestors_ItemChecked(object sender, ItemCheckedEventArgs e)
    {
      if (this.initial || this.readOnly)
        return;
      this.dirty = true;
      HedgeAdvisoryServiceUI tag = (HedgeAdvisoryServiceUI) e.Item.Tag;
    ...
    private void miLinkTo_Click(object sender, EventArgs e)
    {
      ...
      {
        HedgeAdvisoryServiceUI tag = (HedgeAdvisoryServiceUI) selectedItem.Tag;
    ...
    private void lsvInvestors_ItemCheck(object sender, ItemCheckEventArgs e)
    {
      if (this.initial)
        return;
      InvestorServiceAclInfo investorServiceAclInfo = ((HedgeAdvisoryServiceUI) this.lsvInvestors.Items[e.Index].Tag).InvestorServiceAclInfo;
      if (this.readOnly)
      {
        this.dirty = false;
    ...
    private void miDefaultLinkTo_Click(object sender, EventArgs e)
    {
      this.dirty = true;
      this.initial = true;
      switch (this.investorServiceMgr.GetPersonasInvestorServicesDefaultSetting(this.feature, this.userID, HedgeAdvisoryServicePage.investorCategory, this.personaList))
      {
        case InvestorServiceAclInfo.InvestorServicesDefaultSetting.None:
          this.rdbNone.Checked = true;
          break;
      ...
      {
        listViewItem.ImageIndex = 1;
        HedgeAdvisoryServiceUI tag = (HedgeAdvisoryServiceUI) listViewItem.Tag;
        InvestorServiceAclInfo investorServiceAclInfo = tag.InvestorServiceAclInfo;
        if (investorServiceAclInfo != null)
          listViewItem.Checked = investorServiceAclInfo.PersonaAccess == AclResourceAccess.ReadWrite;
        investorServiceAclInfo.CustomAccess = AclResourceAccess.None;
    ...
    private void InitializeComponent()
    {
      ...
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (HedgeAdvisoryServicePage));
      ...
      this.Name = nameof (HedgeAdvisoryServicePage);
      this.ShowIcon = false;
      this.ShowInTaskbar = false;
      this.StartPosition = FormStartPosition.CenterParent;
      this.Text = "Select Hedge Advisory Service";
