...
// Type: EllieMae.EMLite.Setup.AutomatedEnhancedConditionEditor
...
{
  public class AutomatedEnhancedConditionEditor : Form
  {
    private const string className = "AutomatedEnhancedConditionEditor";
    private AutomatedEnhancedConditionRuleInfo ruleInfo;
    ...
    public AutomatedEnhancedConditionEditor(
      AutomatedEnhancedConditionRuleInfo ruleInfo,
      Sessions.Session session)
    {
      ...
      this.ruleCondForm.InitControl(BpmCategory.AutomatedEnhancedConditions);
      this.panelCondition.Controls.Add((Control) this.ruleCondForm);
      this.initForm();
      this.hasNoAccess = !((FeaturesAclManager) session.ACL.GetAclManager(AclCategory.Features)).GetUserApplicationRight(AclFeature.SettingsTab_AutomatedEnhancedConditions);
    ...
    private void initForm()
    {
      ...
      if (this.ruleInfo.Conditions != null && this.ruleInfo.Conditions.Length != 0)
      {
        for (int index = 0; index < this.ruleInfo.Conditions.Length; ++index)
          this.gridViewConditions.Items.Add(new GVItem(this.ruleInfo.Conditions[index].ConditionType)
          {
            SubItems = {
    ...
    public AutomatedEnhancedConditionRuleInfo AutomatedEnhancedCondition => this.ruleInfo;
    ...
    private void okBtn_Click(object sender, EventArgs e)
    {
      ...
      {
        BizRuleInfo[] allRules = ((BpmManager) this.session.BPM.GetBpmManager(BpmCategory.AutomatedEnhancedConditions)).GetAllRules();
        ...
        {
          List<EllieMae.EMLite.ClientServer.AutomatedEnhancedCondition> enhancedConditionList = new List<EllieMae.EMLite.ClientServer.AutomatedEnhancedCondition>();
          for (int nItemIndex = 0; nItemIndex < this.gridViewConditions.Items.Count; ++nItemIndex)
            enhancedConditionList.Add((EllieMae.EMLite.ClientServer.AutomatedEnhancedCondition) this.gridViewConditions.Items[nItemIndex].Tag);
          this.ruleInfo = this.ruleInfo == null ? new AutomatedEnhancedConditionRuleInfo(this.textBoxName.Text.Trim()) : new AutomatedEnhancedConditionRuleInfo(this.ruleInfo.RuleID, this.textBoxName.Text.Trim());
          this.ruleInfo.Conditions = enhancedConditionList.ToArray();
    ...
    {
      using (SelectEnhancedConditionForm enhancedConditionForm = new SelectEnhancedConditionForm(this.session))
      {
        if (enhancedConditionForm.ShowDialog((IWin32Window) this) != DialogResult.OK)
          return;
        string conditionTypeString = enhancedConditionForm.SelectedConditionTypeString;
        List<EnhancedConditionTemplate> selectedConditions = enhancedConditionForm.GetSelectedConditions();
        this.gridViewConditions.BeginUpdate();
        foreach (EnhancedConditionTemplate conditionTemplate in selectedConditions)
        {
          EllieMae.EMLite.ClientServer.AutomatedEnhancedCondition enhancedCondition = new EllieMae.EMLite.ClientServer.AutomatedEnhancedCondition(conditionTypeString, conditionTemplate.Title);
          this.gridViewConditions.Items.Add(new GVItem(conditionTypeString)
          {
            SubItems = {
              (object) conditionTemplate.Title
            },
            Tag = (object) enhancedCondition
          });
        }
        this.gridViewConditions.EndUpdate();
      }
    ...
    private void removeBtn_Click(object sender, EventArgs e)
    {
      ...
      {
        int num = (int) Utils.Dialog((IWin32Window) this, "You have to select an enhanced condition first.", MessageBoxButtons.OK, MessageBoxIcon.Hand);
      }
      else
      {
    ...
    {
      JedHelp.ShowHelp((Control) this, SystemSettings.HelpFile, nameof (AutomatedEnhancedConditionEditor));
    }
    ...
    private void InitializeComponent()
    {
      ...
      this.label2.Size = new Size(363, 13);
      this.label2.TabIndex = 4;
      this.label2.Text = "3. Is there a condition for this Automated Enhanced Conditions";
      this.label1.AutoSize = true;
      this.label1.Font = new Font("Microsoft Sans Serif", 8.25f, FontStyle.Bold, GraphicsUnit.Point, (byte) 0);
      this.label1.Location = new Point(13, 7);
      this.label1.Name = "label1";
      this.label1.Size = new Size(294, 13);
      this.label1.TabIndex = 0;
      this.label1.Text = "1. Create a Automated Enhanced Conditions Name";
      ...
      this.emHelpLink1.HelpTag = "Setup\\Automated Enhanced Conditions";
      ...
      this.Name = nameof (AutomatedEnhancedConditionEditor);
      this.ShowInTaskbar = false;
      this.StartPosition = FormStartPosition.CenterScreen;
      this.Text = "Automated Enhanced Conditions Editor";
