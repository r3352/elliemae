...
// Type: EllieMae.EMLite.Setup.BidTapeServicePage
...
{
  public class BidTapeServicePage : Form
  {
    private static string investorCategory = "BidTape";
    ...
    private Dictionary<string, BidTapeServiceUI> investors = new Dictionary<string, BidTapeServiceUI>();
    ...
    public static string InvestorCategory => BidTapeServicePage.investorCategory;

    public BidTapeServicePage(
    ...
    private void selectAll(bool custom)
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        ...
        if (custom)
        {
          ((BidTapeServiceUI) listViewItem.Tag).InvestorServiceAclInfo.CustomAccess = AclResourceAccess.ReadWrite;
          listViewItem.ImageIndex = 0;
        }
        else
          ((BidTapeServiceUI) listViewItem.Tag).InvestorServiceAclInfo.PersonaAccess = AclResourceAccess.ReadWrite;
      }
      this.dirty = true;
    }
    ...
    private void deselectAll(bool custom)
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        ...
        if (custom)
        {
          ((BidTapeServiceUI) listViewItem.Tag).InvestorServiceAclInfo.CustomAccess = AclResourceAccess.ReadOnly;
          listViewItem.ImageIndex = 0;
        }
        else
          ((BidTapeServiceUI) listViewItem.Tag).InvestorServiceAclInfo.PersonaAccess = AclResourceAccess.ReadOnly;
      }
      this.dirty = true;
    }
    ...
    {
      this.investors = new Dictionary<string, BidTapeServiceUI>();
      int num = 0;
      foreach (PartnerResponseBody partnerResponseBody in new DataDocsServiceHelper(this.session).GetInvestorsList().Where<PartnerResponseBody>((Func<PartnerResponseBody, bool>) (provider => provider.Category.Equals(BidTapeServicePage.investorCategory, StringComparison.InvariantCultureIgnoreCase))))
      {
        string str = partnerResponseBody.PartnerID + partnerResponseBody.ProductName;
        if (!this.investors.ContainsKey(str))
        {
          InvestorServiceAclInfo investorServiceAclInfo = new InvestorServiceAclInfo(str, 9013, BidTapeServicePage.investorCategory);
          BidTapeServiceUI bidTapeServiceUi = new BidTapeServiceUI(investorServiceAclInfo, partnerResponseBody.DisplayName, ++num);
          this.availableServices.Add(investorServiceAclInfo);
          this.investors.Add(str, bidTapeServiceUi);
        }
      }
    }
    ...
    private void loadInvestorServicesForPersona()
    {
      this.initial = true;
      this.dirty = false;
      InvestorServiceAclInfo.InvestorServicesDefaultSetting servicesDefaultSetting = this.investorServiceMgr.GetInvestorServicesDefaultSetting(this.feature, this.personaID, BidTapeServicePage.investorCategory);
      this.previousSettings = servicesDefaultSetting;
      InvestorServiceAclInfo[] permissions = this.investorServiceMgr.GetPermissions(this.feature, this.personaID, BidTapeServicePage.investorCategory, this.availableServices.ToArray());
      this.lsvInvestors.Items.Clear();
      if (this.investors != null && this.investors.Count > 0)
        this.lsvInvestors.ListViewItemSorter = (IComparer) this.investors.Values.First<BidTapeServiceUI>();
      foreach (InvestorServiceAclInfo investorServiceAclInfo in permissions)
      {
        BidTapeServiceUI bidTapeServiceUi = (BidTapeServiceUI) null;
        this.investors.TryGetValue(investorServiceAclInfo.ProviderCompanyCode, out bidTapeServiceUi);
        if (bidTapeServiceUi != null)
        {
          bidTapeServiceUi.InvestorServiceAclInfo = investorServiceAclInfo;
          ListViewItem listViewItem = new ListViewItem(bidTapeServiceUi.DisplayName);
          listViewItem.Tag = (object) bidTapeServiceUi;
    ...
    private void loadInvestorServicesForUser()
    {
      this.initial = true;
      this.dirty = false;
      InvestorServiceAclInfo.InvestorServicesDefaultSetting servicesDefaultSetting = this.investorServiceMgr.GetUserSpecificInvestorServicesDefaultSetting(this.feature, this.userID, BidTapeServicePage.investorCategory, this.personaList);
      InvestorServiceAclInfo[] permissions = this.investorServiceMgr.GetPermissions(this.feature, this.userID, BidTapeServicePage.investorCategory, this.personaList, this.availableServices.ToArray());
      this.lsvInvestors.Items.Clear();
      if (this.investors != null && this.investors.Count > 0)
        this.lsvInvestors.ListViewItemSorter = (IComparer) this.investors.Values.First<BidTapeServiceUI>();
      foreach (InvestorServiceAclInfo investorServiceAclInfo in permissions)
      ...
      if (servicesDefaultSetting == InvestorServiceAclInfo.InvestorServicesDefaultSetting.NotSpecified)
      {
        ...
        servicesDefaultSetting = this.investorServiceMgr.GetInvestorServicesDefaultSetting(this.feature, this.userID, BidTapeServicePage.investorCategory, this.personaList);
      }
      else
    ...
    public void SaveData()
    {
      ...
      foreach (ListViewItem listViewItem in this.lsvInvestors.Items)
      {
        InvestorServiceAclInfo investorServiceAclInfo = ((BidTapeServiceUI) listViewItem.Tag).InvestorServiceAclInfo;
      ...
      if (!this.isPersonal)
      {
        this.investorServiceMgr.SetPermissions(this.feature, investorServiceAclInfoList.ToArray(), this.personaID, BidTapeServicePage.investorCategory, defaultValue);
        this.investorServiceMgr.SetDefaultValue(this.feature, this.personaID, BidTapeServicePage.investorCategory, defaultValue);
      }
      else
      {
        if (this.lblDefaultSetting.ImageIndex == 1)
          defaultValue = InvestorServiceAclInfo.InvestorServicesDefaultSetting.NotSpecified;
        this.investorServiceMgr.SetPermissions(this.feature, investorServiceAclInfoList.ToArray(), this.userID, BidTapeServicePage.investorCategory, defaultValue);
        this.investorServiceMgr.SetDefaultValue(this.feature, this.userID, BidTapeServicePage.investorCategory, defaultValue);
      }
      this.dirty = false;
    }
    ...
    private void lsvInvestors_ItemChecked(object sender, ItemCheckedEventArgs e)
    {
      if (this.initial || this.readOnly)
        return;
      this.dirty = true;
      BidTapeServiceUI tag = (BidTapeServiceUI) e.Item.Tag;
    ...
    private void miLinkTo_Click(object sender, EventArgs e)
    {
      ...
      {
        BidTapeServiceUI tag = (BidTapeServiceUI) selectedItem.Tag;
    ...
    private void lsvInvestors_ItemCheck(object sender, ItemCheckEventArgs e)
    {
      if (this.initial)
        return;
      InvestorServiceAclInfo investorServiceAclInfo = ((BidTapeServiceUI) this.lsvInvestors.Items[e.Index].Tag).InvestorServiceAclInfo;
      if (this.readOnly)
      {
        this.dirty = false;
    ...
    private void miDefaultLinkTo_Click(object sender, EventArgs e)
    {
      this.dirty = true;
      this.initial = true;
      switch (this.investorServiceMgr.GetPersonasInvestorServicesDefaultSetting(this.feature, this.userID, BidTapeServicePage.investorCategory, this.personaList))
      {
        case InvestorServiceAclInfo.InvestorServicesDefaultSetting.None:
          this.rdbNone.Checked = true;
          break;
      ...
      {
        listViewItem.ImageIndex = 1;
        BidTapeServiceUI tag = (BidTapeServiceUI) listViewItem.Tag;
        InvestorServiceAclInfo investorServiceAclInfo = tag.InvestorServiceAclInfo;
        if (investorServiceAclInfo != null)
          listViewItem.Checked = investorServiceAclInfo.PersonaAccess == AclResourceAccess.ReadWrite;
        investorServiceAclInfo.CustomAccess = AclResourceAccess.None;
    ...
    private void InitializeComponent()
    {
      ...
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (BidTapeServicePage));
      ...
      this.Name = nameof (BidTapeServicePage);
      this.ShowIcon = false;
      this.ShowInTaskbar = false;
      this.StartPosition = FormStartPosition.CenterParent;
      this.Text = "Select Bid Tape Firm";
