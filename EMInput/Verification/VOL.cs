...
// Type: EllieMae.EMLite.Verification.VOL
...
{
  public class VOL : VerificationBase
  {
    protected int ind;
    protected bool firstTime = true;

    public VOL(PanelBase p, IMainScreen mainScreen, IWorkArea area, LoanData loanData)
      : base("Liability", mainScreen, area, loanData)
    {
      this.selPanel = p;
      this.verType = nameof (VOL);
    }

    public VOL(PanelBase p, IMainScreen mainScreen, IWorkArea area)
      : base("Liability", mainScreen, area)
    {
      this.selPanel = p;
      this.verType = nameof (VOL);
    }

    public VOL(string title, IMainScreen mainScreen, IWorkArea area)
      : base(title, mainScreen, area)
    {
    }

    public VOL(string title, IMainScreen mainScreen, IWorkArea area, LoanData loanData)
    ...
    protected virtual int GetNumberOfVerification()
    {
      return this.loan.GetNumberOfLiabilitesExlcudingAlimonyJobExp();
    }
    ...
    protected virtual void RemoveVerification(int i) => this.loan.RemoveLiabilityAt(i);
