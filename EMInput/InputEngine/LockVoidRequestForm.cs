...
// Type: EllieMae.EMLite.InputEngine.LockVoidRequestForm
...
{
  public class LockVoidRequestForm : UserControl, IRefreshContents
  {
    ...
    private TextBox txtVoidComments;
    private TextBox txtVoidDate;
    private Label label2;
    private Label label1;

    public LockVoidRequestForm(Sessions.Session session, LoanData loan)
    {
      this.session = session;
      this.loan = loan;
      this.InitializeComponent();
      LockRequestLog lockRequestLog = LockUtils.GetLastLockRequestForVoid(this.session.LoanDataMgr);
      if (lockRequestLog == null)
      {
        LockRequestLog currentLockRequest = this.loan.GetLogList().GetCurrentLockRequest();
        Hashtable hashtable = currentLockRequest != null ? currentLockRequest.GetLockRequestSnapshot() : throw new Exception("Cannot void lock without a confirmed lock.");
        if (hashtable == null)
          throw new Exception("Cannot void lock without a confirmed lock (A).");
        if (string.IsNullOrEmpty((string) hashtable[(object) "3839"]))
          throw new Exception("Cannot void lock without a confirmed lock (B).");
      ...
      this.txtVoidDate.Text = DateTime.Now.ToShortDateString();
      this.txtVoidComments.Text = "";
    ...
    public bool RequestLockVoid(bool interactive)
    {
      this.loan.SetField(string.Concat(this.txtVoidDate.Tag), this.txtVoidDate.Text.Trim());
      this.loan.SetField(string.Concat(this.txtVoidComments.Tag), this.txtVoidComments.Text.Trim());
      try
      {
        this.session.LoanDataMgr.CreateVoidLockRequest(this.session.UserInfo, Utils.ParseDate((object) this.txtVoidDate, DateTime.MinValue), this.txtVoidComments.Text.Trim());
      }
      catch (Exception ex)
      {
        if (!interactive)
      ...
      this.txtVoidDate.Text = "";
      this.loan.SetField(string.Concat(this.txtVoidDate.Tag), "");
      this.txtVoidComments.Text = "";
      this.loan.SetField(string.Concat(this.txtVoidComments.Tag), "");
      ...
      if (interactive)
      {
        ...
        {
          int num1 = (int) Utils.Dialog((IWin32Window) this.session.MainForm, "Void has been successfully confirmed.", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
        }
        else
        {
          int num2 = (int) Utils.Dialog((IWin32Window) this, "Void has been submitted successfully.", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
        }
      }
      return true;
    }
    ...
    private void InitializeComponent()
    {
      ...
      this.txtVoidComments = new TextBox();
      this.txtVoidDate = new TextBox();
      ...
      this.groupContainer1.Controls.Add((Control) this.txtVoidComments);
      this.groupContainer1.Controls.Add((Control) this.txtVoidDate);
      ...
      this.groupContainer1.Text = "Void Information";
      this.txtVoidComments.Location = new Point(108, 58);
      this.txtVoidComments.Multiline = true;
      this.txtVoidComments.Name = "txtVoidComments";
      this.txtVoidComments.Size = new Size(216, 66);
      this.txtVoidComments.TabIndex = 3;
      this.txtVoidComments.Tag = (object) "4686";
      this.txtVoidDate.Location = new Point(108, 32);
      this.txtVoidDate.Name = "txtVoidDate";
      this.txtVoidDate.ReadOnly = true;
      this.txtVoidDate.Size = new Size(216, 20);
      this.txtVoidDate.TabIndex = 2;
      this.txtVoidDate.Tag = (object) "4687";
