...
// Type: EllieMae.EMLite.MainUI.ULDDExportDialog
...
{
  public class ULDDExportDialog : Form
  {
    ...
    private static ULDDExportDialog instance;
    ...
    private const string className = "ULDDExportDialog";
    ...
    public static ULDDExportDialog Instance
    {
      get
      {
        if (ULDDExportDialog.instance == null)
          ULDDExportDialog.instance = new ULDDExportDialog();
        return ULDDExportDialog.instance;
      }
    }

    public static List<string> GuidList => ULDDExportDialog.guidList;

    static ULDDExportDialog()
    {
      if (Session.Connection.IsServerInProcess)
        a.a("mkl9m3X90nM45sY");
      else
        a.a("i9w9j72bidpm93x");
      if (ULDDExportDialog.jed != null)
        return;
      if (Session.Connection.IsServerInProcess)
        ULDDExportDialog.jed = a.b("mk0T8jLZ0");
      else
        ULDDExportDialog.jed = a.b("km0w2khs9");
    }

    private ULDDExportDialog()
    {
      ...
      Tracing.Log(ULDDExportDialog.sw, nameof (ULDDExportDialog), TraceLevel.Info, "Starting AutoSave timer...");
    ...
    private void autosaveHandler(object sender, EventArgs e)
    {
      ...
      {
        Tracing.Log(ULDDExportDialog.sw, nameof (ULDDExportDialog), TraceLevel.Error, "Error during autosave: " + (object) ex);
      }
    }
    ...
    private string getAutoRecoverFilePath(LoanDataMgr loanDataMgr)
    {
      if (loanDataMgr == null || loanDataMgr.LoanFolder == null)
        return (string) null;
      if (ULDDExportDialog.autosaveFolderFixedPart == null)
      {
        ...
        try
        {
          ...
          if (registryKey != null)
            ULDDExportDialog.autosaveFolderFixedPart = (string) registryKey.GetValue("AutosaveFolderRoot");
        }
        catch
        {
        }
        ...
        if (ULDDExportDialog.autosaveFolderFixedPart != null)
          ULDDExportDialog.autosaveFolderFixedPart = ULDDExportDialog.autosaveFolderFixedPart.Trim();
        if ((ULDDExportDialog.autosaveFolderFixedPart ?? "") == "")
          ULDDExportDialog.autosaveFolderFixedPart = EnConfigurationSettings.GlobalSettings.AppLoanAutosaveDirectory;
        string str = "(local)";
        if (!Session.Connection.IsServerInProcess)
          str = Session.CompanyInfo.ClientID;
        ULDDExportDialog.autosaveFolderFixedPart = Path.Combine(ULDDExportDialog.autosaveFolderFixedPart, str + "\\" + Session.UserID);
      }
      string path = Path.Combine(ULDDExportDialog.autosaveFolderFixedPart, loanDataMgr.LoanFolder);
      ...
      string exportType)
    {
      ...
      ULDDExportDialog.passDataChecking = true;
    ...
    public void RefreshPipeline(bool preserveSelection)
    {
      ...
      {
        using (PerformanceMeter performanceMeter = PerformanceMeter.StartNew("Export.Pipeline.Refresh", "Refresh the pipeline screen data", true, 336, nameof (RefreshPipeline), "D:\\ws\\24.3.0.0\\EmLite\\EMLite\\MainUI\\ULDDExportDialog.cs"))
        {
    ...
    private bool retrievePipelineData(QueryCriterion filter, SortField[] sortFields)
    {
      ...
      {
        MetricsFactory.IncrementErrorCounter(ex, "Error loading pipeline", "D:\\ws\\24.3.0.0\\EmLite\\EMLite\\MainUI\\ULDDExportDialog.cs", nameof (retrievePipelineData), 405);
    ...
    {
      List<string> stringList = new List<string>((IEnumerable<string>) ULDDExportDialog.requiredFields);
    ...
    private void displayCurrentPage(bool preserveSelections)
    {
      ...
      if (ULDDExportDialog.guidList == null)
        ULDDExportDialog.guidList = new List<string>();
      else
        ULDDExportDialog.guidList.Clear();
    ...
    private GVItem createGVItemForPipelineInfo(PipelineInfo pinfo, LoanInfo.Right effectiveRights)
    {
      ...
      for (int index = 0; index < this.gvLoans.Columns.Count; ++index)
      {
        ...
        else if (LoanDataMgr.OpenLoan(Session.SessionObjects, pinfo.GUID, false).LoanData.Calculator.ULDDExportChecking(this.exportType).Count > 0)
        {
          obj = (object) "Problem";
          ULDDExportDialog.passDataChecking = false;
        }
        else
          obj = (object) "OK";
        itemForPipelineInfo.SubItems[index].Value = obj;
        itemForPipelineInfo.Tag = (object) pinfo;
        if (!ULDDExportDialog.guidList.Contains(pinfo.GUID))
          ULDDExportDialog.guidList.Add(pinfo.GUID);
    ...
    public static bool PassDataCheck => ULDDExportDialog.passDataChecking;
    ...
    {
      using (PerformanceMeter performanceMeter = PerformanceMeter.StartNew("Loan.Open", "Opening a loan file", true, true, 693, nameof (openLoan), "D:\\ws\\24.3.0.0\\EmLite\\EMLite\\MainUI\\ULDDExportDialog.cs"))
      {
    ...
    private void startEditor(LoanDataMgr loanDataMgr)
    {
      ...
      loanDataMgr.LoanData.ULDDExportType = this.exportType;
    ...
    private void recoverAutoSavedFile(LoanDataMgr loanDataMgr)
    {
      ...
      try
      {
        ...
        using (FileStream A_0 = File.OpenRead(autoRecoverFileName))
          xmlData = ULDDExportDialog.jed.a((Stream) A_0);
      ...
      try
      {
        ...
        using (FileStream A_0 = File.OpenRead(recoverAttFileName))
          attXml = ULDDExportDialog.jed.a((Stream) A_0);
      }
      catch
      {
      }
      ...
      try
      {
        ...
        using (FileStream A_0 = File.OpenRead(recoverHistoryFileName))
          hisXml = ULDDExportDialog.jed.a((Stream) A_0);
      }
      catch
      {
      }
    ...
    private LoanDataMgr openLoanDataMgr(string guid)
    {
      ...
      {
        MetricsFactory.IncrementErrorCounter((Exception) ex, "The selected loan has been deleted or is no longer accessible", "D:\\ws\\24.3.0.0\\EmLite\\EMLite\\MainUI\\ULDDExportDialog.cs", nameof (openLoanDataMgr), 1284);
    ...
    {
      using (PerformanceMeter performanceMeter = PerformanceMeter.StartNew("Loan.Save", "Saving a loan file to the Encompass Server", true, 1460, nameof (SaveLoan), "D:\\ws\\24.3.0.0\\EmLite\\EMLite\\MainUI\\ULDDExportDialog.cs"))
      {
        ...
        using (MetricsFactory.GetIncrementalTimer("LoanSaveIncTimer", (SFxTag) new SFxInternalTag()))
          flag = Session.LoanDataMgr.SaveLoan(mergeOnly, true, false, milestoneTemplateController, false, out bool _);
        if (refreshScreen)
    ...
    private void ULDDExportDialog_FormClosing(object sender, FormClosingEventArgs e)
    {
      if (this.hasOpenLoan && !this.CloseLoan(true))
        return;
      ULDDExportDialog.instance = (ULDDExportDialog) null;
    }
    ...
    private void InitializeComponent()
    {
      ...
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (ULDDExportDialog));
      ...
      this.loanTabPage.Size = new Size(851, 420);
      ...
      this.label1.Size = new Size(772, 13);
      this.label1.TabIndex = 0;
      this.label1.Text = "There is incomplete or invalid data in one or more loan files.  See the Export Status column values to find the export problems.  Open the loan file to make changes.";
      ...
      this.Name = nameof (ULDDExportDialog);
