...
// Type: EllieMae.EMLite.MainUI.ThinThick.TradingScreens.Trades
...
{
  public class Trades : ITradingList, ITradingScreen
  {
    ...
    public Trades()
    {
      ...
      this.tradeFieldDefs = TradeReportFieldDefs.GetFieldDefs();
    }

    public ToolStripItemCollection ToolStripItemCollection => MainForm.Instance.TradingMenu_Trades;

    public string MenuName => "Trades - TradeList";
    ...
    public int ItemCount => this.Ids != null ? this.Ids.Length : 0;

    public ListValueCriterion FilterById
    {
      get => new ListValueCriterion("LoanTradeDetails.TradeID", (Array) this.Ids);
    }
    ...
    public QueryCriterion SubViewCriterion
    {
      get
      {
        return this.CurrentOrArchived == CurrentOrArchived.Current ? (QueryCriterion) new ListValueCriterion("LoanTradeDetails.Status", (Array) new int[5]
        {
          2,
          0,
          1,
          4,
          3
        }, true) : (QueryCriterion) new ListValueCriterion("LoanTradeDetails.Status", (Array) new int[1]
        {
          5
        }, true);
      }
    ...
    public ICursor GetTradingCursor(bool exportAll)
    {
      ...
      if (this.Filter == null)
        return Session.LoanTradeManager.OpenTradeCursor(new QueryCriterion[0], this.SortFields, this.Fields, true, false);
      return Session.LoanTradeManager.OpenTradeCursor(new QueryCriterion[1]
      {
        this.Filter
    ...
    public OpSimpleResponse ExportToExcel(bool exportAll, bool print)
    {
      ...
      LoanTradeViewModel[] items = (LoanTradeViewModel[]) this.GetTradingCursor(exportAll).GetItems(0, itemCount);
      List<List<string>> dataList = new List<List<string>>();
      foreach (LoanTradeViewModel loanTradeViewModel in items)
      {
        List<string> stringList = new List<string>();
        ...
        while (enumerator2.MoveNext())
        {
          ...
          string str = loanTradeViewModel[current.Tag] == null ? string.Empty : loanTradeViewModel[current.Tag].ToString();
