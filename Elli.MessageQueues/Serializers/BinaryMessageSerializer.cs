// Decompiled with JetBrains decompiler
// Type: Elli.MessageQueues.Serializers.BinaryMessageSerializer
// Assembly: Elli.MessageQueues, Version=1.0.0.0, Culture=neutral, PublicKeyToken=d11ef57bba4acf91
// MVID: 24211DB4-B81B-430D-BE95-0449CADCF25D
// Assembly location: C:\SmartClientCache\Apps\UAC\Ellie Mae\xIHR5EqGa7zPnRG0YpD5z4TPAB0=\Encompass360\Elli.MessageQueues.dll

using System;
using System.IO;
using System.Runtime.Serialization.Formatters.Binary;

#nullable disable
namespace Elli.MessageQueues.Serializers
{
  public class BinaryMessageSerializer : ISerializer
  {
    public byte[] Serialize<T>(T message)
    {
      if ((object) message == null)
        throw new ArgumentNullException("BinarySerializer.Serialize(message)");
      using (MemoryStream serializationStream = new MemoryStream())
      {
        new BinaryFormatter().Serialize((Stream) serializationStream, (object) message);
        return serializationStream.GetBuffer();
      }
    }

    public T Deserialize<T>(byte[] bytes)
    {
      if (bytes == null)
        throw new ArgumentNullException("BinarySerializer.Deserialize(bytes)");
      using (MemoryStream serializationStream = new MemoryStream(bytes))
        return (T) new BinaryFormatter().Deserialize((Stream) serializationStream);
    }
  }
}
