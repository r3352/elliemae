...
// Type: Elli.Server.Remoting.Cursors.GseCommitmentCursor
...
{
  public class GseCommitmentCursor : CursorBase
  {
    private const string className = "GseCommitmentCursor";
    private bool summaryOnly;
    private string[] fields;
    private UserInfo userInfo;

    public GseCommitmentCursor Initialize(
      ISession session,
      int[] contractIds,
      string[] fields,
      bool summaryOnly)
    ...
    private void insertPrimaryKey()
    {
      ...
      if (this.fields != null)
      {
        ...
        {
          if (field == "GseCommitmentDetails.TradeID")
          {
            flag = true;
            break;
          }
      ...
      if (!flag)
        stringList.Add("GseCommitmentDetails.TradeID");
      this.fields = stringList.ToArray();
    }

    public override object[] GetItems(int startIndex, int count)
    {
      TraceLog.WriteApi(nameof (GseCommitmentCursor), nameof (GetItems), (object) startIndex, (object) count);
      int[] array = (int[]) this.Items.GetRange(startIndex, count).ToArray(typeof (int));
      return this.summaryOnly ? (object[]) new List<GSECommitmentViewModel>((IEnumerable<GSECommitmentViewModel>) GseCommitments.GetTradeViews(array).Values).ToArray() : (object[]) GseCommitments.GetTrades(array);
    }
