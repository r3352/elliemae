...
// Type: Elli.Server.Remoting.Cursors.MbsPoolCursor
...
{
  public class MbsPoolCursor : CursorBase
  {
    private const string className = "MbsPoolCursor";
    private bool summaryOnly;
    private string[] fields;
    private UserInfo userInfo;

    public MbsPoolCursor Initialize(
      ISession session,
      int[] contractIds,
      string[] fields,
      bool summaryOnly)
    ...
    private void insertPrimaryKey()
    {
      ...
      if (this.fields != null)
      {
        ...
        {
          if (field == "MbsPoolDetails.TradeID")
          {
            flag = true;
            break;
          }
      ...
      if (!flag)
        stringList.Add("MbsPoolDetails.TradeID");
      this.fields = stringList.ToArray();
    }

    public override object[] GetItems(int startIndex, int count)
    {
      TraceLog.WriteApi(nameof (MbsPoolCursor), nameof (GetItems), (object) startIndex, (object) count);
      int[] array = (int[]) this.Items.GetRange(startIndex, count).ToArray(typeof (int));
      return this.summaryOnly ? (object[]) new List<MbsPoolViewModel>((IEnumerable<MbsPoolViewModel>) MbsPools.GetTradeViews(array).Values).ToArray() : (object[]) LoanTrades.GetTrades(array);
    }
