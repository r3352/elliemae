...
// Type: Elli.Server.Remoting.Cursors.SecurityTradeCursor
...
{
  public class SecurityTradeCursor : CursorBase
  {
    private const string className = "SecurityTradeCursor";
    private bool summaryOnly;
    private string[] fields;
    private UserInfo userInfo;

    public SecurityTradeCursor Initialize(
      ISession session,
      int[] contractIds,
      string[] fields,
      bool summaryOnly)
    ...
    private void insertPrimaryKey()
    {
      ...
      if (this.fields != null)
      {
        ...
        {
          if (field == "SecurityTradeDetails.TradeID")
          {
            flag = true;
            break;
          }
      ...
      if (!flag)
        stringList.Add("SecurityTradeDetails.TradeID");
      this.fields = stringList.ToArray();
    }

    public override object[] GetItems(int startIndex, int count)
    {
      TraceLog.WriteApi(nameof (SecurityTradeCursor), nameof (GetItems), (object) startIndex, (object) count);
      int[] array = (int[]) this.Items.GetRange(startIndex, count).ToArray(typeof (int));
      return this.summaryOnly ? (object[]) new List<SecurityTradeViewModel>((IEnumerable<SecurityTradeViewModel>) SecurityTrades.GetTradeViews(array).Values).ToArray() : (object[]) SecurityTrades.GetTrades(array);
    }
