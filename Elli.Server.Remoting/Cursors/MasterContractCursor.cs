...
// Type: Elli.Server.Remoting.Cursors.MasterContractCursor
...
{
  public class MasterContractCursor : CursorBase
  {
    private const string className = "MasterContractCursor";
    private bool summaryOnly;
    private string[] fields;
    private UserInfo userInfo;

    public MasterContractCursor Initialize(
      ISession session,
      int[] contractIds,
      string[] fields,
      bool summaryOnly)
    ...
    private void insertPrimaryKey()
    {
      ...
      if (this.fields != null)
      {
        ...
        {
          if (field == "MasterContracts.ContractID")
          {
            flag = true;
            break;
          }
      ...
      if (!flag)
        stringList.Add("MasterContracts.ContractID");
      this.fields = stringList.ToArray();
    }

    public override object[] GetItems(int startIndex, int count)
    {
      TraceLog.WriteApi(nameof (MasterContractCursor), nameof (GetItems), (object) startIndex, (object) count);
      int[] array = (int[]) this.Items.GetRange(startIndex, count).ToArray(typeof (int));
      return this.summaryOnly ? (object[]) MasterContracts.GetContractSummaries(array) : (object[]) MasterContracts.GetContracts(array);
    }
