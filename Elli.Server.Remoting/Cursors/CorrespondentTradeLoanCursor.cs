...
// Type: Elli.Server.Remoting.Cursors.CorrespondentTradeLoanCursor
...
{
  public class CorrespondentTradeLoanCursor : CursorBase
  {
    private const string className = "CorrespondentTradeLoanCursor";
    private CorrespondentTradeInfo[] trades;
    private string[] fields;
    private PipelineData dataToInclude;

    public CorrespondentTradeLoanCursor Initialize(
      ISession session,
      int[] tradeIds,
      string[] fields,
    ...
    {
      return this.Initialize(session, CorrespondentTrades.GetTrades(tradeIds), fields, dataToInclude, sortOrder, isExternalOrganization, filterOption);
    }
      ...
      EligibleLoanFilterOption filterOption)
    {
      ...
      PipelineInfo[] pinfos = CorrespondentTrades.GetEligibleLoans(session.GetUserInfo(), trades, new string[0], sortOrder, isExternalOrganization, filterOption);
    ...
    {
      TraceLog.WriteApi(nameof (CorrespondentTradeLoanCursor), nameof (GetItems), (object) startIndex, (object) count);
      string[] array = (string[]) this.Items.GetRange(startIndex, count).ToArray(typeof (string));
      PipelineInfo[] source1 = Pipeline.Generate(this.Session.GetUserInfo(), array, this.fields, this.dataToInclude, isExternalOrganization, tradeType: TradeType.CorrespondentTrade);
      ...
      foreach (PipelineInfo pinfo in source2)
        CorrespondentTrades.PopulateTradeProfitData(pinfo, this.trades);
